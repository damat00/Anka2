...
void CMiniGame::AttendanceMonsterAttack(LPCHARACTER pkAttacker, LPCHARACTER pkVictim)
{
	...
	if(pkAttacker->GetRaceNum() >= 6500 && pkAttacker->GetRaceNum() <= 6504)
	{
		if(pkVictim->IsPC())
		{
	...
	if(pkVictim->GetRaceNum() >= 6500 && pkVictim->GetRaceNum() <= 6504)
	{
		bool bContain = false;
		DWORD dwCount = 1;
...
void CMiniGame::AttendanceEventRequestReward(LPCHARACTER pChar)
{
	...
	{
		pChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Bu etkinlik þu anda aktif."));
		return;
	}
	
	if(pChar->GetQuestFlag(szClearName) == 0)
	{
		pChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Bu ödülü alabilmek için önce hak etmen gerekiyor."));
		return;
	}
	
	if(pChar->GetQuestFlag(szRewardName))
	{
		pChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Bugün için ödülünü zaten aldýn."));
		return;
	}
	
	if(pChar->GetLevel() < 30)
	{
		pChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Bu etkinlik için en az 30 seviye olman gerekiyor."));
		return;
	}
...
bool CMiniGame::ReadRewardItemFile(const char * c_pszFileName)
{
	...
	if (!fp)
	{
		sys_err("ReadRewardItemFile: cannot open file %s", c_pszFileName);
		return false;
	}

	// Önce mevcut vector'ü temizle ve reserve et
	attendanceRewardVec.clear();
	attendanceRewardVec.reserve(365); // Yýllýk maksimum gün sayýsý için reserve

	char szLine[512];
	int iLineNum = 0;
	unsigned int uiDay, uiVnum, uiCount;
	
	while (fgets(szLine, sizeof(szLine), fp))
	{
		iLineNum++;
		
		// Satýr sonu karakterlerini temizle
		char *pNewline = strchr(szLine, '\n');
		if (pNewline)
			*pNewline = '\0';
		pNewline = strchr(szLine, '\r');
		if (pNewline)
			*pNewline = '\0';
		
		// Boþ satýrlarý ve yorum satýrlarýný atla
		if (szLine[0] == '\0' || szLine[0] == '#')
			continue;
			
		// Format: day vnum count (tab veya space ile ayrýlmýþ)
		// Geçici unsigned int kullan, sonra BYTE'a cast et
		// sscanf format string'inde whitespace karakterlerini otomatik olarak atlar
		// %u formatý baþta whitespace'leri atlar, ardýndan integer okur, sonra whitespace atlar
		int iReadCount = sscanf(szLine, "%u%*[\t ]%u%*[\t ]%u", &uiDay, &uiVnum, &uiCount);
		
		if (iReadCount != 3)
		{
			// Alternatif: space ile dene
			iReadCount = sscanf(szLine, "%u %u %u", &uiDay, &uiVnum, &uiCount);
		}
		
		if (iReadCount != 3)
		{
			// Debug: satýr içeriðini hex olarak göster
			char szDebug[1024];
			char *pDebug = szDebug;
			for (size_t i = 0; i < strlen(szLine) && i < 100; i++)
			{
				sprintf(pDebug, "%02X ", (unsigned char)szLine[i]);
				pDebug += 3;
			}
			sys_err("ReadRewardItemFile: invalid format in %s line %d (read %d values, expected 3)", c_pszFileName, iLineNum, iReadCount);
			sys_err("ReadRewardItemFile: line content: [%s] hex: [%s]", szLine, szDebug);
			fclose(fp);
			attendanceRewardVec.clear();
			return false;
		}

		// Deðer aralýklarýný kontrol et
		if (uiDay == 0 || uiDay > 255)
	{
			sys_err("ReadRewardItemFile: invalid day value %u in %s line %d (must be 1-255)", uiDay, c_pszFileName, iLineNum);
			fclose(fp);
			attendanceRewardVec.clear();
			return false;
		}

		if (!ITEM_MANAGER::instance().GetTable(uiVnum))
		{
			sys_err("ReadRewardItemFile: no such item (Vnum: %lu) in %s line %d", uiVnum, c_pszFileName, iLineNum);
			fclose(fp);
			attendanceRewardVec.clear();
			return false;
		}
		...
		tempItem.bDay = (BYTE)uiDay;
		tempItem.dwVnum = uiVnum;
		tempItem.dwCount = uiCount;

		attendanceRewardVec.push_back(tempItem);
	}
	
	fclose(fp);
	sys_log(0, "ReadRewardItemFile: successfully loaded %lu items from %s", attendanceRewardVec.size(), c_pszFileName);
	return true;
}
