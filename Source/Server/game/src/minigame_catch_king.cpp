...
void CMiniGame::MiniGameCatchKingStartGame(LPCHARACTER pkChar, BYTE bSetCount)
{
	...
	{
		pkChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Bahis yapýlan set sayýsý geçersiz, lütfen tekrar deneyin."));
		return;
	}
	
	if (pkChar->GetGold() < (30000 * bSetCount))
	{
		pkChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Bu oyunu baþlatmak için yeterli yangýnýz yok."));
		return;
	}
	
	if (pkChar->CountSpecifyItem(79604) < bSetCount)
	{
		pkChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Bu oyunu baþlatmak için yeterli kart setiniz yok."));
		return;
	}
	...
	std::shuffle(m_vecFieldCards.begin(), m_vecFieldCards.end(), rng);
...
void CMiniGame::MiniGameCatchKingRegisterScore(LPCHARACTER pkChar, DWORD dwScore)
{
	...
	snprintf(querySelect, sizeof(querySelect), 
		"SELECT max_score FROM log.catch_king_event WHERE name = '%s' LIMIT 1;", pkChar->GetName());
	...
	if (resSelect && resSelect->uiNumRows > 0)
	{
		...
		if (row)
		{
			...
			if(dwScore > maxScore)
				DBManager::instance().DirectQuery("UPDATE log.catch_king_event SET max_score = %d, total_score = total_score + %d WHERE name = '%s';", dwScore, dwScore, pkChar->GetName());
			else
				DBManager::instance().DirectQuery("UPDATE log.catch_king_event SET total_score = total_score + %d WHERE name = '%s';", dwScore, pkChar->GetName());
		}
		else
		{
			sys_log(0, "MiniGameCatchKingRegisterScore: mysql_fetch_row returned NULL for %s", pkChar->GetName());
		}
	}
	else
	{
		DBManager::instance().DirectQuery("REPLACE INTO log.catch_king_event (name, empire, max_score, total_score) VALUES ('%s', %d, %d, %d);", 
...
int CMiniGame::MiniGameCatchKingGetScore(lua_State* L, bool isTotal)
{
	...
	{
		snprintf(querySelect, sizeof(querySelect), 
			"SELECT name, empire, total_score FROM log.catch_king_event ORDER BY total_score DESC LIMIT 10;");
	}
	else
	{
		snprintf(querySelect, sizeof(querySelect), 
			"SELECT name, empire, max_score FROM log.catch_king_event ORDER BY max_score DESC LIMIT 10;");
	}
	
	std::unique_ptr<SQLMsg> pSelectMsg(DBManager::instance().DirectQuery(querySelect));
	if (!pSelectMsg)
	{
		sys_log(0, "MiniGameCatchKingGetScore: DirectQuery returned NULL");
		return 0;
	}
	...
	if (!resSelect)
	{
		sys_log(0, "MiniGameCatchKingGetScore: pSelectMsg->Get() returned NULL");
		return 0;
	}
	
	if (!resSelect->pSQLResult)
	{
		sys_log(0, "MiniGameCatchKingGetScore: resSelect->pSQLResult is NULL");
		return 0;
	}
	
	// uiNumRows kontrolü: LIMIT 10 kullandýðýmýz için maksimum 10 satýr olmalý
	// Ancak uiNumRows bozuk olabilir, bu yüzden hem uiNumRows hem de maksimum 10 ile sýnýrla
	uint uiMaxRows = 10; // LIMIT 10 kullandýðýmýz için maksimum 10
	if (resSelect->uiNumRows > 0 && resSelect->uiNumRows < uiMaxRows)
		uiMaxRows = resSelect->uiNumRows;
	
	// uiNumRows çok büyükse (bozuk), sadece 10 ile sýnýrla
	if (resSelect->uiNumRows > 1000)
	{
		sys_log(0, "MiniGameCatchKingGetScore: uiNumRows is suspiciously large (%u), limiting to 10", resSelect->uiNumRows);
		uiMaxRows = 10;
	}
	
	// Güvenli loop: maksimum 10 iterasyon, her iterasyonda result set kontrolü
	// pSelectMsg'ý scope içinde tutmak için yerel deðiþkene kopyala
	MYSQL_RES* pResult = resSelect->pSQLResult;
	
	for (uint i = 0; i < uiMaxRows && i < 10; i++)
	{
		// Result set'in hala geçerli olduðundan emin ol
		if (!pResult)
		{
			sys_log(0, "MiniGameCatchKingGetScore: pResult became NULL during iteration at row %u", i);
			break;
		}
		
		// mysql_fetch_row çaðrýsýndan önce result set kontrolü
		MYSQL_ROW row = mysql_fetch_row(pResult);
		if (!row)
		{
			// NULL döndü, daha fazla satýr yok veya hata oluþtu
			// MySQL error kontrolü
			break;
		}
		
		// row[0], row[1], row[2] NULL kontrolü
		if (!row[0] || !row[1] || !row[2])
		{
			sys_log(0, "MiniGameCatchKingGetScore: row[%u] has NULL values, skipping", i);
			continue;
		}
	...
	// Lua fonksiyonu table döndürüyor, 1 deðer döndürüldüðünü belirt
	return 1;
}
...
int CMiniGame::MiniGameCatchKingGetMyScore(LPCHARACTER pkChar)
{
	...
	snprintf(querySelect, sizeof(querySelect), "SELECT max_score FROM log.catch_king_event WHERE name = '%s' LIMIT 1;", pkChar->GetName());
	...
	if (resSelect && resSelect->uiNumRows > 0)
	{
		...
		{
			sys_log(0, "MiniGameCatchKingGetMyScore: mysql_fetch_row returned NULL for %s", pkChar->GetName());
		}
	}
