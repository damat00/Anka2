// ========================================================================
// $File: //jeffr/granny_29/rt/granny_camera.cpp $
...
#define SubsystemCode CameraLogMessage
...
InitializeDefaultCamera(camera &Camera)
{
    ...
    Camera.MirrorXInView = false;
    Camera.MirrorYInView = false;
    Camera.MirrorZInView = false;
...
void GRANNY
SetCameraFOVX(camera& Camera, real32 FOVX)
{
    CheckBoundedReal32(0.0f, FOVX, Pi32, return);

    real32 OppX = ASin(FOVX/2);
    real32 OppY = OppX / Camera.WwOverHw;

    Camera.FOVY = 2.0f * ATan2(OppY, 1.0f);
}
...
BuildViewMatrix(camera &Camera, real32 *Matrix, real32 *InverseMatrix)
{
    ...
    // Do mirroring if requested
    {
        matrix_3x3 Mirror;
        ZeroStructure(Mirror);
        if (Camera.MirrorXInView)
        {
            Mirror[0][0] = -1;
            Mirror[1][1] =  1;
            Mirror[2][2] =  1;
            MatrixMultiply3x3((real32 *)Temp, (real32 const*)Axes, (real32 const*)Mirror);
            MatrixEquals3x3((real32*)Axes, (real32*)Temp);
        }
        if (Camera.MirrorYInView)
        {
            Mirror[0][0] =  1;
            Mirror[1][1] = -1;
            Mirror[2][2] =  1;
...
WindowSpaceToWorldSpace(camera const &Camera,
                        ...
                        real32 *Result)
{
    // Note out of order set to segregate Z-wackiness.
    quad Homogenous;
    Homogenous[0] = (2.0f*ScreenSpacePoint[0] - Width) / Width;
    Homogenous[1] = (2.0f*ScreenSpacePoint[1] - Height) / Height;
    Homogenous[3] = 1.0f;

    // Account for the z-range of the camera
    if(Camera.OutputZRange == CameraOutputZNegativeOneToOne)
    {
        real32 const ZMin = -1.0f;
        real32 const ZMax = 1.0f;
        Homogenous[2] = (ScreenSpacePoint[2] - ZMin) / (ZMax - ZMin);
    }
    else if (Camera.OutputZRange == CameraOutputZZeroToOne)
    {
        // Already in device coords.  See below.
        Homogenous[2] = ScreenSpacePoint[2];
    }
    else
    {
        Assert(Camera.OutputZRange == CameraOutputZNegativeOneToZero);
        // Already in device coords.  See below.
        Homogenous[2] = ScreenSpacePoint[2];
    }


    TransposeVectorTransform4x4(Homogenous, (real32 const *)Camera.InverseProjection4x4);
    TransposeVectorTransform4x4(Homogenous, (real32 const *)Camera.InverseView4x4);
                        ...
                        real32 *Result)
{
    ...
    {
        // D3D Mode.
        // No need to modify this - it's already in device coordinates [0,1] For D3D, the
        // app will need to apply the D3DVIEWPORT.MinZ and .MaxZ scaling itself, since the
        // granny_camera doesn't get told about those.
    ...
    else
    {
        ...
        // Wii-TF Mode.  Already in device coords, they're just /weird/ device coords.
        Result[2] = Homogenous[2];
    }
}
...
GetMostLikelyPhysicalAspectRatio(int32x ScreenWidth, int32x ScreenHeight)
{
    ...
    {
        return VerticalIphoneAspectRatio;
    }
    else
    {
