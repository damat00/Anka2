// ========================================================================
// $File: //jeffr/granny_29/rt/granny_mesh_builder.cpp $
// $DateTime: 2012/03/13 17:38:10 $
// $Change: 36757 $
// $Revision: #4 $
//
...
#include "granny_mesh_builder_internal.h"
...
{
    {for(int32x Edge = 0; Edge < 3; ++Edge)
    {
        Builder.MaterialIndex = 0;
        Builder.VertexIndex[Edge] = -1;
...
{
    Assert(MaterialCount > 0 || (VertexCount == 0 && TriangleCount == 0));
...
#define MySpecialAggrAllocOffsetSizePtr(Allocator, OwnerPointer,Size,Member, Intent)    \
    {                                                                                   \
        void *TempPointer = AllocateSize ( Size, Intent );                              \
        *((void**)&(OwnerPointer->Member)) = TempPointer;                               \
        Log2(NoteLogMessage, ExporterLogMessage,                                        \
             "Allocate %i bytes for " #Member " got address 0x%x", Size, TempPointer ); \
        FailedToAllocate |= ( TempPointer == NULL );                                    \
    } typedef int __RequireSemiColon


#define MySpecialAggrAllocOffsetArrayPtr(Allocator, OwnerPointer,Count,CountMember,PtrMember, Intent) \
    {                                                                                               \
        void *TempPointer = AllocateSize ( Count * sizeof(OwnerPointer->PtrMember[0]), Intent );    \
        *((void**)&(OwnerPointer->PtrMember)) = TempPointer;                                        \
        OwnerPointer->CountMember = Count;                                                          \
        Log2(NoteLogMessage, ExporterLogMessage,                                                    \
             "Allocate %i bytes for " #PtrMember " got address 0x%x",                               \
             Count * sizeof(OwnerPointer->PtrMember[0]), TempPointer );                             \
        FailedToAllocate |= ( TempPointer == NULL );                                                \
    } typedef int __RequireSemiColon
...
#define MySpecialAggrAllocOffsetCountlessArrayPtr(Allocator, OwnerPointer,Count, PtrMember, Intent) \
    {                                                                                               \
        void *TempPointer = AllocateSize ( Count * sizeof(OwnerPointer->PtrMember[0]), Intent );    \
        *((void**)&(OwnerPointer->PtrMember)) = TempPointer;                                        \
        FailedToAllocate |= ( TempPointer == NULL );                                                \
    ...
    mesh_builder *Builder;
    {
        ...
        EndAggrAlloc(Allocator, AllocationBuilder);
    }
...
#define MySpecialAggrAllocOffsetCountlessArrayPtr AggrAllocOffsetCountlessArrayPtr
    ...
    MySpecialAggrAllocOffsetArrayPtr(Allocator, Builder, BoneCount,BoneCount, BoneWeights, AllocationBuilder);
    MySpecialAggrAllocOffsetArrayPtr(Allocator, Builder, MaterialCount, MaterialCount,
                                     FirstMaterialTriangle, AllocationBuilder);
    MySpecialAggrAllocOffsetCountlessArrayPtr(Allocator, Builder, EdgeHashTableSize, EdgeHashHeads, AllocationBuilder);
    ...
    MySpecialAggrAllocOffsetSizePtr(Allocator, Builder, BufferedVertexSize, BufferedVertex[0], AllocationBuilder);
    MySpecialAggrAllocOffsetSizePtr(Allocator, Builder, BufferedVertexSize, BufferedVertex[1], AllocationBuilder);
    MySpecialAggrAllocOffsetSizePtr(Allocator, Builder, BufferedVertexSize, BufferedVertex[2], AllocationBuilder);

    MySpecialAggrAllocOffsetSizePtr(Allocator, Builder, VertexSize, TruncatedVertex, AllocationBuilder);
    MySpecialAggrAllocOffsetSizePtr(Allocator, Builder, BufferedVertexSize, ComparisonVertex, AllocationBuilder);
    ...
    MySpecialAggrAllocOffsetSizePtr(Allocator, Builder,
                                    ...
                                    BufferedVertexType, AllocationBuilder);
    ...
    MySpecialAggrAllocOffsetArrayPtr ( Allocator, Builder, ThisVertexComponentCount,
                                       VertexComponentNameCount, VertexComponentNames, AllocationBuilder );

    MySpecialAggrAllocOffsetCountlessArrayPtr(Allocator, Builder,
                                              MaximumTriangleCount, OrigTriIndexToFinalIndex, AllocationBuilder);
    ...
    AnyFailed = !StackInitializeWithDirectory(Builder->VertexStack, VertexSize, GeneralBlockSize, MaximumVertexCount) || AnyFailed;
    AnyFailed = !StackInitializeWithDirectory(Builder->NextCoincidentVertStack, SizeOf(int32), GeneralBlockSize, MaximumVertexCount) || AnyFailed;
    AnyFailed = !StackInitializeWithDirectory(Builder->FromTriangleStack, SizeOf(int32), GeneralBlockSize, MaximumVertexCount) || AnyFailed;
    AnyFailed = !StackInitializeWithDirectory(Builder->FromPolygonStack, SizeOf(int32), GeneralBlockSize, MaximumVertexCount) || AnyFailed;
    AnyFailed = !StackInitializeWithDirectory(Builder->FromPolygonIndexStack, SizeOf(int32), GeneralBlockSize, MaximumVertexCount) || AnyFailed;
    AnyFailed = !StackInitializeWithDirectory(Builder->NextInMaterialStack, SizeOf(int32), GeneralBlockSize, MaximumTriangleCount) || AnyFailed;
    AnyFailed = !StackInitializeWithDirectory(Builder->VertexIndexStack, SizeOf(int32), GeneralBlockSize, (3*MaximumTriangleCount)) || AnyFailed;
    AnyFailed = !StackInitializeWithDirectory(Builder->TriangleTangentStack, SizeOf(tangent_frame), GeneralBlockSize, MaximumTriangleCount) || AnyFailed;
    AnyFailed = !StackInitializeWithDirectory(Builder->VertexTangentStack,   SizeOf(tangent_frame), GeneralBlockSize, MaximumVertexCount) || AnyFailed;
    AnyFailed = !StackInitializeWithDirectory(Builder->EdgeStack, SizeOf(builder_edge), GeneralBlockSize, MaximumEdgeCount) || AnyFailed;
    ...
    if ( Builder )
    {
        ...
        Builder->MaterialIndex = 0;
        Builder->Polygon = -1;
        SetInt32(3, -1, Builder->PolygonIndex);
        SetInt32(3, -1, Builder->VertexIndex);

        Builder->AllowTangentVertSplits = true;
        Builder->PreserveEmptyMaterialGroups = false;
        ...
        SetInt32(MaterialCount, Terminator, Builder->FirstMaterialTriangle);

        // Neg out the original index -> final index map
        SetInt32(MaximumTriangleCount, Terminator, Builder->OrigTriIndexToFinalIndex);
        ...
        Assert(Index == 0 || VertexCount == 0);
        MultipleNewStackUnits(Builder->FromTriangleStack, VertexCount, Index, &Terminator);
        Assert(Index == 0 || VertexCount == 0);

        MultipleNewStackUnits(Builder->FromPolygonStack, VertexCount, Index, &Terminator);
        Assert(Index == 0 || VertexCount == 0);
        MultipleNewStackUnits(Builder->FromPolygonIndexStack, VertexCount, Index, &Terminator);
        Assert(Index == 0 || VertexCount == 0);
        ...
        {
            Assert(Builder->VertexComponentNames != 0);
        ...
        int32x V0, int32x V1)
{
    ...
    int32x NewEdgeIndex;
    if (NewStackUnit(Builder.EdgeStack, &NewEdgeIndex) == 0)
    {
        // oh, very, very bad.
        Assert(false);
        return;
...
static uint32
GetNextEdgeAndTriangle(mesh_builder &Builder, uint32 FromTriangleAndEdge, int32x V0, int32x V1)
{
    uint32 const HashValue      = GetEdgeHashValue(V0, V1);
    uint32 const HashTableIndex = HashValue % EdgeHashTableSize;
...
static int32x
BuildIndices(mesh_builder &Builder, int32x FirstTriangle, int32x BaseIndex, int32 *IndexDataOut)
{
    int32x Count = 0;
    ...
    while(TriangleIndex != Terminator)
    {
        ...
        *IndexDataOut++ = STACK_UNIT_AS(Builder.VertexIndexStack, 3*TriangleIndex + 2, int32);
        ...
        Builder.OrigTriIndexToFinalIndex[TriangleIndex] = BaseIndex + Count;
             ...
             tri_topology *&Topology)
{
    ...
    if (Builder.PreserveEmptyMaterialGroups)
    {
        UsedMaterialCount = Builder.MaterialCount;
    }
...
void DeallocateSafeBuilder ( mesh_builder *Builder )
{
    if ( Builder != NULL )
    {
        ...
        StackCleanUp ( Builder->FromPolygonStack );
        StackCleanUp ( Builder->FromPolygonIndexStack );
        ...
        DeallocateSafe ( Builder->OrigTriIndexToFinalIndex );
    #endif
    ...
    if(Builder)
    {
        ...
        if(EndAggrPlacement(Allocator, VertexDataMemory))
        {
            ...
            if(EndAggrPlacement(Allocator, TopologyMemory))
            {
                ...
                {for(int32x MaterialIndex = 0; MaterialIndex < Builder->MaterialCount; ++MaterialIndex)
                {
                    int32x const FirstTriangle =
                        Builder->FirstMaterialTriangle[MaterialIndex];

                    if (Builder->PreserveEmptyMaterialGroups ||
                        FirstTriangle != Terminator)
                    {
                        tri_material_group &Group = Topology->Groups[GroupIndex++];

                        Group.MaterialIndex = MaterialIndex;
                        Group.TriFirst = TriangleCount;
                        Group.TriCount =
                            BuildIndices(*Builder, FirstTriangle, TriangleCount, &V[TriangleCount * 3]);
...
void GRANNY
DisableTangentVertexSplits(mesh_builder& Builder)
{
    Builder.AllowTangentVertSplits = false;
}

void GRANNY
PreserveEmptyMaterialGroups(mesh_builder& Builder)
{
    Builder.PreserveEmptyMaterialGroups = true;
}
                    ...
                    real32 Tolerance)
{
    ...
    Assert(Channel < MaximumChannelCount);
...
static int32x
GetMatchingVertex(mesh_builder &Builder,
                  int32x TriangleIndex,
                  int32x Side,
                  int32x PolygonIndex,
                  int32x PolygonSide)
{
    ...
    Assert(VertexIndex >= 0 && VertexIndex < Builder.VertexCount);
    if(STACK_UNIT_AS(Builder.NextCoincidentVertStack, VertexIndex, int32) != Terminator)
    {
        ...
        NewStackUnit(Builder.FromTriangleStack, &CheckIndex);     Assert(CheckIndex == VertexIndex);
        NewStackUnit(Builder.FromPolygonStack, &CheckIndex);      Assert(CheckIndex == VertexIndex);
        NewStackUnit(Builder.FromPolygonIndexStack, &CheckIndex); Assert(CheckIndex == VertexIndex);
    }
    ...
    STACK_UNIT_AS(Builder.FromPolygonStack, VertexIndex, int32)                = PolygonIndex;
    STACK_UNIT_AS(Builder.FromPolygonIndexStack, VertexIndex, int32)           = PolygonSide;
...
ClipWeights(mesh_builder &Builder)
{
    ...
    if(WeightSum > 0.0f)
    {
        ...
        {for(int32x WeightIndex = 1;
             ...
             ++WeightIndex)
        {
            if (TopWeights[WeightIndex] == 0)
                TopIndices[WeightIndex] = TopIndices[0];
        }}
    }
    else
    {
        if(WeightCount)
        {
            // This can happen now that you may select "always skinned" in
            // the exporter.  Just use 0 as the index and 1 as the weight.
...
PushVertex(mesh_builder &Builder)
{
    ...
    ConvertSingleObject(P3VertexType,
                        Builder.Point, Builder.VertexType,
                        VertexPointer,
                        VertexMemberConverter);
    MergeSingleObject(VertexWeightArraysType,
                      &Builder.WeightArrays, Builder.VertexType,
                      VertexPointer,
                      VertexMemberConverter);

    variant Weights;
    if (FindMatchingMember(Builder.VertexType, 0, VertexBoneWeightsName, &Weights))
    {
...
{
    Assert(Edge < 3);
    Assert(Index >= 0 && Index < Builder.VertexCount);
    Builder.VertexIndex[Edge] = Index;
}

void GRANNY
SetPolygonForVertex(mesh_builder& Builder, int32x Polygon)
{
    Builder.Polygon = Polygon;
}

void GRANNY
SetPolygonIndexForVertex(mesh_builder& Builder, int32x Edge, int32x Index)
{
    Assert(Edge >= 0 && Edge < 3);
    Builder.PolygonIndex[Edge] = Index;
}
...
SetChannel(mesh_builder &Builder, int32x Edge, int32x ChannelIndex,
           real32 const *Value, int32x ValueWidth)
{
    Assert(Edge >= 0 && Edge < 3);
    Assert(ChannelIndex >= 0);
    Assert(ChannelIndex < (int32x)Builder.ChannelCount);
    ...
    Assert(ValueWidth <= ChannelDef->ArrayWidth);

    for (int32x Idx = 0; Idx < ValueWidth; ++Idx)
        *Channel++ = *Value++;
    for (int32x Idx = ValueWidth; Idx < ChannelDef->ArrayWidth; ++Idx)
        *Channel++ = 0;
}
...
PushTriangle(mesh_builder &Builder)
{
    ...
    {for(int32x Side = 0; Side < 3; ++Side)
    {
        STACK_UNIT_AS(Builder.VertexIndexStack, FirstVertIndex + Side, int32) =
            GetMatchingVertex(Builder,
                              TriangleIndex, Side,
                              Builder.Polygon, Builder.PolygonIndex[Side]);
    }}
...
{
    Assert(VertexNumber >= 0 && VertexNumber < GetStackUnitCount(Builder.VertexStack));
...
{
    Assert(VertexIndex >= 0 && VertexIndex < GetStackUnitCount(Builder.NextCoincidentVertStack));
...
{
    Assert(VertexIndex >= 0 && VertexIndex < GetStackUnitCount(Builder.FromTriangleStack));
    return STACK_UNIT_AS(Builder.FromTriangleStack, VertexIndex, int32);
}

void GRANNY
GetFromPolygonAndIndexForVertex(mesh_builder &Builder,
                                int32x VertexIndex,
                                int32x& Polygon,
                                int32x& Side)
{
    Assert(GetStackUnitCount(Builder.FromPolygonStack) ==
           GetStackUnitCount(Builder.FromPolygonIndexStack));
    Assert(VertexIndex >= 0 && VertexIndex < GetStackUnitCount(Builder.FromPolygonStack));

    int32x OrigCap = GetStackUnitCount(Builder.FromPolygonStack);
    int32x Walk = VertexIndex;
    while (Walk >= OrigCap)
    {
        Walk = GetNextCoincidentVertex(Builder, Walk);
        Assert(Walk >= 0);
        Assert(Walk != VertexIndex);
    }

    Polygon = STACK_UNIT_AS(Builder.FromPolygonStack,      Walk, int32);
    Side    = STACK_UNIT_AS(Builder.FromPolygonIndexStack, Walk, int32);
}
