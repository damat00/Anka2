// ========================================================================
// $File: //jeffr/granny_29/rt/granny_curve_fast.cpp $
...
UnsignedShortToFloat(uint16 Val)
{
    real32 High = ByteToFloatTable[Val >> 8];
    real32 Low  = ByteToFloatTable[Val & 0xFF];
                                ...
                                real32 const* NOALIAS IdentityVector)
{
    ...
    if ( ControlResults != NULL )
    {
        ...
        {for ( int32x Count = 0; Count < KnotCount; Count++ )
        {
            ...
            bool MissingComponentIsNegative = ( CurControl[0] & 0x80 ) != 0;
            int32x MissingComponentIndex    = ( (( CurControl[1] >> 6 ) & 0x2 ) |
                                                (CurControl[2] >> 7) );
            ...
            {for ( int32x SrcComp = 0; SrcComp < 3; SrcComp++ )
            {
                ...
                real32 Result = (ControlOffsets[DstComp] +
                                 ControlScales[DstComp] * ByteToFloatTable[CurControl[SrcComp] & 0x7f]);
...
void GRANNY
CurveExtractKnotValuesD4nK16uC15u(curve2 const &Curve,
                                  int32x KnotIndex,
                                  int32x KnotCount,
                                  real32* NOALIAS KnotResults,
                                  real32* NOALIAS ControlResults,
...
{
    Assert(CurveFormatIsInitializedCorrectly(Curve, false));
    curve_data_d4n_k16u_c15u const &CurveData =
        *(curve_data_d4n_k16u_c15u*)Curve.CurveData.Object;

    int32x CurveKnotCount = (CurveData.KnotControlCount >> 2);
    Assert ( KnotIndex >= 0 );
    Assert ( KnotIndex + KnotCount <= CurveKnotCount );

    const uint16* NOALIAS CurKnot    = CurveData.KnotsControls + KnotIndex;
    const uint16* NOALIAS CurControl = CurveData.KnotsControls + CurveKnotCount + KnotIndex * 3;
    ...
    if (KnotResults != NULL)
    {
        ...
        {for ( int32x Count = 0; Count < KnotCount; Count++ )
        {
            *KnotResults++ = KnotScale * UnsignedShortToFloat(*CurKnot);
            ++CurKnot;
        }}
    }
    ...
    if ( ControlResults != NULL )
    {
        ...
        {for ( int32x Count = 0; Count < KnotCount; Count++ )
        {
            ...
            bool MissingComponentIsNegative = ( CurControl[0] & 0x8000 ) != 0;
            int32x MissingComponentIndex = ( ( CurControl[1] >> 14 ) & 0x2 ) | ( CurControl[2] >> 15 );
            ...
            {for ( int32x SrcComp = 0; SrcComp < 3; SrcComp++ )
            {
                ...
                real32 Result = (ControlOffsets[DstComp] +
                                 ControlScales[DstComp] * UnsignedShortToFloat(CurControl[SrcComp] & 0x7fff));
                Assert ( AbsoluteValue ( Result ) < 0.71f );
                SummedSq += Result * Result;
                ControlResults[DstComp] = Result;
            }}
