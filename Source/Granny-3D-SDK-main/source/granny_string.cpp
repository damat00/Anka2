// ========================================================================
// $File: //jeffr/granny_29/rt/granny_string.cpp $
...
#define SubsystemCode StringLogMessage
...
// Needs to be a CALLBACK_FN to be passed around
CALLBACK_FN(int32x) GRANNY
StringDifferenceInternal(char const *StringA, char const *StringB)
{
    if ( StringA == NULL )
    {
        if ( StringB == NULL )
...
{
    return StringDifferenceInternal(StringA, StringB);
}

int32x GRANNY
StringDifferenceLowercase(char const *StringA, char const *StringB)
{
    if ( StringA == NULL )
    {
        if ( StringB == NULL )
    ...
    else
    {
        ...
        else
        {
            ...
            return (ConvertToLowercase(*StringA) - ConvertToLowercase(*StringB));
        }
    }
}
...
bool GRANNY
StringsAreEqualLowercaseNoSlash(char const *StringA, char const *StringB)
{
    if(StringA && StringB)
    {
        while(*StringA && *StringB &&
              ((ConvertToLowercase(*StringA) == ConvertToLowercase(*StringB)) ||
               (IsSlash(*StringA) && IsSlash(*StringB))))
        {
            ++StringA;
            ++StringB;
        }
...
char const* GRANNY
StringContains(char const* String,
               char const* Substring)
{
    // Sorta valid, but hard to deal with properly.  Return 0 in all cases.
...
char const* GRANNY
StringContainsLowercase(char const* String,
                        char const* Substring)
{
    // Sorta valid, but hard to deal with properly.  Return 0 in all cases.
    if (String == 0 || Substring == 0)
...
char const *GRANNY
FindLastInstanceOf(char const *String, char Char)
{
    char const *Result = 0;
    while(*String != '\0')
    {
...
CloneString(char const *Source)
{
    ...
    if(Source)
    {
        ...
        Result = AllocateArray(Length, char, AllocationUnknown);
...
int32 GRANNY
ConvertToInt32(char const* String)
{
    // TODO: Use Real64's once there's a conversion routine
    return RoundReal64ToInt32(ConvertToReal32(String, false));
}

uint32 GRANNY
ConvertToUInt32(char const* String)
{
    // TODO: Use Real64's once there's a conversion routine
...
real32 GRANNY
ConvertToReal32(char const* String, bool const AllowFractions)
{
    return(real32(ConvertToReal64(String, AllowFractions)));
}

real64x GRANNY
ConvertToReal64(char const* StringInit, bool const AllowFractions)
{
    real64x Negate = 1.0;
