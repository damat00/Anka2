...
element * listPrevious( list * l, element * current );
...
(*REQUIRED IF USING FIND*)  #define CONTAINER_COMPARE_FIND_FIELDS( item ) stricmp( key, (item)->name )
...
// This can't go inside a push/pop since it is checked for at the end of the compilation unit.
...
#ifndef CONTAINER_NO_UNDEFINE
#define CONTAINER_NO_UNDEFINE 0
#endif

#ifndef CONTAINER_FAVOR_RIGHT
#define CONTAINER_FAVOR_RIGHT 0
#endif
...
// this should be an int the same size as a pointer
  ...
  #endif
#else
  // Note that this line only detects X64 on mac.  Linux/etc? still needed.
  // The second clause detects NaCl, which is x64 with 32 bit pointers.                                                            
    ...
    #define CONTAINER_INTPTR long long
  #else
...
#if CONTAINER_DO_ALLOCATION
  #undef SET_UNUSED_COUNT
  #undef GET_UNUSED_COUNT
  #undef SET_UNUSED_NEXT
  #undef GET_UNUSED_NEXT
...
static void MakeNamePost( get_and_remove ) ( CONTAINER_NAME * sl, CONTAINER_ITEM_TYPE * remove_item )
{
  ...
  for (;;)
  {
    ...
    {
      CONTAINER_ITEM_TYPE * if_right;

      #if CONTAINER_NEED_EXACT_DUPE_REMOVAL
        remove_right:
      #endif

      if_right = GET_RIGHT( i );
...
CONTAINER_FUNCTION_DECORATE( int ) MakeNamePost( Initialize ) ( CONTAINER_NAME * sl
#if CONTAINER_DO_ALLOCATION
,int HintItems )
#else
) 
...
#if CONTAINER_EMIT_CODE
{
    ...
    return (sl->CONTAINER_UNUSED_MEMBER != 0);
  #else
    return 1;
  #endif
}
...
#ifdef CONTAINER_FAVOR_RIGHT
  #undef CONTAINER_FAVOR_RIGHT
#endif
