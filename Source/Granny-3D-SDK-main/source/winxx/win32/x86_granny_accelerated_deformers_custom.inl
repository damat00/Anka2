...
#ifndef X86_GRANNY_ASM_HELPER_H
#include "x86_granny_asm_helper.h"
#endif
...
transform_loop:
        ...
        lea edi, STRUCTACCESS(esi, pwnt3432_vertex, Normal)

        // Store our tex coordinates in xmm5
        movlps xmm5, STRUCTACCESS(esi, pwnt3432_vertex, UV)
        ...
        mov eax, STRUCTACCESS(ebx, SSEABUF, TransformTable)
#endif
        xor ecx, ecx
        mov cl, STRUCTACCESS(esi, pwnt3432_vertex, BoneIndices[0])
#ifndef DIRECT_ROUTINES
        mov ecx, [eax+ecx*4]
#endif
        shl ecx, 6 // matrix_4x4 is 64 bytes
        add ecx, STRUCTACCESS(ebx, SSEABUF, Transforms)
        xor edx, edx
        mov dl, STRUCTACCESS(esi, pwnt3432_vertex, BoneIndices[1])
#ifndef DIRECT_ROUTINES
        mov edx, [eax+edx*4]
#endif
        shl edx,6
        add edx, STRUCTACCESS(ebx, SSEABUF, Transforms)

        // set up our boneweights
        xor eax, eax
        mov al, STRUCTACCESS(esi, pwnt3432_vertex, BoneWeights[0])
        cvtsi2ss xmm6, eax
        mulss xmm6, [OneOver255]
        shufps xmm6, xmm6, 0

        mov al, STRUCTACCESS(esi, pwnt3432_vertex, BoneWeights[1])
        cvtsi2ss xmm7, eax
    ...
    __asm
    {
        ...
        movaps STRUCTACCESS(ebx, SSEABUF, DestPosXYZ), xmm0
        movaps STRUCTACCESS(ebx, SSEABUF, DestNormXYZ), xmm1
...
#endif
        xor ecx, ecx
        mov cl, STRUCTACCESS(esi, pwnt3432_vertex, BoneIndices[2])
#ifndef DIRECT_ROUTINES
        mov ecx, [eax+ecx*4]
#endif
        shl ecx, 6
        add ecx, STRUCTACCESS(ebx, SSEABUF, Transforms)
        xor edx, edx
        mov dl, STRUCTACCESS(esi, pwnt3432_vertex, BoneIndices[3])
#ifndef DIRECT_ROUTINES
        mov edx, [eax+edx*4]
#endif
        shl edx,6
        ...
        // set up our boneweights
        xor eax, eax
        mov al, STRUCTACCESS(esi, pwnt3432_vertex, BoneWeights[2])
        cvtsi2ss xmm6, eax
        mulss xmm6, [OneOver255]
        shufps xmm6, xmm6, 0

        mov al, STRUCTACCESS(esi, pwnt3432_vertex, BoneWeights[3])
        cvtsi2ss xmm7, eax
        mulss xmm7, [OneOver255]
        shufps xmm7, xmm7, 0
    }
    ...
    __asm
    {
        ...
        addps xmm0, STRUCTACCESS(ebx, SSEABUF, DestPosXYZ)
        addps xmm1, STRUCTACCESS(ebx, SSEABUF, DestNormXYZ)
        ...
        // Next
        add [Dest], ASMSIZE(pnt332_vertex)

        add esi, ASMSIZE(pwnt3432_vertex)
        prefetchnta [esi + 3 * ASMSIZE(pwnt3432_vertex)]
...
transform_loop:
        ...
        // set up the pointer to our Normal
        lea edi, STRUCTACCESS(esi, pwnt3232_vertex, Normal)

        // Store our tex coordinates in mm0
        movlps xmm5, STRUCTACCESS(esi, pwnt3232_vertex, UV)
...
#endif
        xor ecx, ecx
        mov cl, STRUCTACCESS(esi, pwnt3232_vertex, BoneIndices[0])
#ifndef DIRECT_ROUTINES
        mov ecx, [eax+ecx*4]
#endif
        shl ecx, 6 // matrix_4x4 is 64 bytes
        add ecx, STRUCTACCESS(ebx, SSEABUF, Transforms)
        xor edx, edx
        mov dl, STRUCTACCESS(esi, pwnt3232_vertex, BoneIndices[1])
#ifndef DIRECT_ROUTINES
        mov edx, [eax+edx*4]
#endif
        shl edx,6
    ...
    __asm
    {
        ...
        add esi, ASMSIZE(pwnt3232_vertex)
        prefetchnta [esi + 3 * ASMSIZE(pwnt3232_vertex)]
...
SseDeformW1Helper()
{
    ...
    __asm
    {
        ...
        movss xmm4, STRUCTACCESS(esi, pwnt3132_vertex, Position[0])    // xx|xx|xx|Px
        shufps xmm4, xmm4, 0                            // Px|Px|Px|Px
        mulps xmm4, [ecx]                               // Px|Px|Px|Px * 03|02|01|00
        addps xmm0, xmm4

        movss xmm4, STRUCTACCESS(esi, pwnt3132_vertex, Position[4])    // xx|xx|xx|Py
        shufps xmm4, xmm4, 0                            // Py|Py|Py|Py
        mulps xmm4, [ecx+16]                            // Py|Py|Py|Py * 13|12|11|10
        addps xmm0, xmm4

        movss xmm4, STRUCTACCESS(esi, pwnt3132_vertex, Position[8])    // xx|xx|xx|Pz
        shufps xmm4, xmm4, 0                            // Pz|Pz|Pz|Pz
        mulps xmm4, [ecx+32]                            // Pz|Pz|Pz|Pz * 23|22|21|20
        addps xmm0, xmm4

        // Normal
        movss xmm1, STRUCTACCESS(esi, pwnt3132_vertex, Normal[0])      // xx|xx|xx|Nx
        shufps xmm1, xmm1, 0                            // Nx|Nx|Nx|Nx
        mulps xmm1, [ecx]                               // Nx|Nx|Nx|Nx * 03|02|01|00

        movss xmm4, STRUCTACCESS(esi, pwnt3132_vertex, Normal[4])      // xx|xx|xx|Ny
        shufps xmm4, xmm4, 0                            // Ny|Ny|Ny|Ny
        mulps xmm4, [ecx+16]                            // Ny|Ny|Ny|Ny * 13|12|11|10
        addps xmm1, xmm4

        movss xmm4, STRUCTACCESS(esi, pwnt3132_vertex, Normal[8])      // xx|xx|xx|Nz
        shufps xmm4, xmm4, 0                            // Nz|Nz|Nz|Nz
        mulps xmm4, [ecx+32]                            // Nz|Nz|Nz|Nz * 23|22|21|20
        addps xmm1, xmm4
        ...
        movss xmm4, STRUCTACCESS(edi, pwnt3132_vertex, Position[0])    // xx|xx|xx|Px
        shufps xmm4, xmm4, 0                            // Px|Px|Px|Px
        mulps xmm4, [edx]                               // Px|Px|Px|Px * 03|02|01|00
        addps xmm2, xmm4

        movss xmm4, STRUCTACCESS(edi, pwnt3132_vertex, Position[4])    // xx|xx|xx|Py
        shufps xmm4, xmm4, 0                            // Py|Py|Py|Py
        mulps xmm4, [edx+16]                            // Py|Py|Py|Py * 13|12|11|10
        addps xmm2, xmm4

        movss xmm4, STRUCTACCESS(edi, pwnt3132_vertex, Position[8])    // xx|xx|xx|Pz
        shufps xmm4, xmm4, 0                            // Pz|Pz|Pz|Pz
        mulps xmm4, [edx+32]                            // Pz|Pz|Pz|Pz * 23|22|21|20
        addps xmm2, xmm4

        // Normal
        movss xmm3, STRUCTACCESS(edi, pwnt3132_vertex, Normal[0])      // xx|xx|xx|Nx
        shufps xmm3, xmm3, 0                            // Nx|Nx|Nx|Nx
        mulps xmm3, [edx]                               // Nx|Nx|Nx|Nx * 03|02|01|00

        movss xmm4, STRUCTACCESS(edi, pwnt3132_vertex, Normal[4])      // xx|xx|xx|Ny
        shufps xmm4, xmm4, 0                            // Ny|Ny|Ny|Ny
        mulps xmm4, [edx+16]                            // Ny|Ny|Ny|Ny * 13|12|11|10
        addps xmm3, xmm4

        movss xmm4, STRUCTACCESS(edi, pwnt3132_vertex, Normal[8])      // xx|xx|xx|Nz
        shufps xmm4, xmm4, 0                            // Nz|Nz|Nz|Nz
        mulps xmm4, [edx+32]                            // Nz|Nz|Nz|Nz * 23|22|21|20
        addps xmm3, xmm4
...
transform_loop:
        ...
        lea edi, [esi + ASMSIZE(pwnt3132_vertex)]

        // Store our tex coordinates in xmm6 and 7
        movlps xmm6, STRUCTACCESS(esi, pwnt3132_vertex, UV) 
        movlps xmm7, STRUCTACCESS(edi, pwnt3132_vertex, UV) 
        ...
        mov eax, STRUCTACCESS(ebx, SSEABUF, TransformTable) 
#endif
        mov ecx, STRUCTACCESS(esi, pwnt3132_vertex, BoneIndex) 
#ifndef DIRECT_ROUTINES
        mov ecx, [eax+ecx*4]
#endif
        shl ecx, 6 // matrix_4x4 is 64 bytes
        ...
        mov edx, STRUCTACCESS(edi, pwnt3132_vertex, BoneIndex) 
#ifndef DIRECT_ROUTINES
        mov edx, [eax+edx*4]
#endif
    ...
    __asm
    {
        ...
        // Next
        add [Dest], (2*ASMSIZE(pnt332_vertex))
        add esi, (2*ASMSIZE(pwnt3132_vertex))

        prefetchnta [esi + 3 * ASMSIZE(pwnt3132_vertex)]
        prefetchnta [esi + 4 * ASMSIZE(pwnt3132_vertex)]
