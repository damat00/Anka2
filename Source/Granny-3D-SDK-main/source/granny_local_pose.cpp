// ========================================================================
// $File: //jeffr/granny_29/rt/granny_local_pose.cpp $
...
data_type_definition LocalPoseType[] =
{
    ...
    { EmptyReferenceMember, "NextPose" },

    { EndMember }
};

struct local_pose_frame_stats
{
    int32x RefPoseFills;
    int32x Lerps;
    int32x RefPoseCopies;
    int32x AccumScales;
};
local_pose_frame_stats g_LocalPoseFrameStats = { 0 };
...
{
    CheckInt32Index(BoneIndex, LocalPose.BoneCount, return(0));
...
{
    CheckInt32Index(FirstBone, LocalPose.BoneCount, return);
    CheckCondition(BoneCount >= 0, return);
    CheckCondition(FirstBone + BoneCount <= LocalPose.BoneCount, return);
...
{
    EndLocalPoseAccumulationLOD(LocalPose,
                                FirstBone, BoneCount,
                                FirstBone, BoneCount,
...
EndLocalPoseAccumulationLOD(local_pose &LocalPose,
                            int32x FirstBone, int32x BoneCount,
                            int32x FirstValidBone, int32x ValidBoneCount,
                            skeleton const &ReferenceSkeleton,
                            real32 AllowedError,
                            int32x const *SparseBoneArray)
{
    ...
    CheckBoundedInt32(0, FirstValidBone + ValidBoneCount, LocalPose.BoneCount, return);

    int32x const ValidEnd = FirstValidBone + ValidBoneCount;
         ...
         ++LocalPoseBoneIndex)
    {
        ...
        if (Local->TraversalID == LocalPose.TraversalID && LocalPoseBoneIndex < ValidEnd)
        {
            if (Local->Weight < LocalPose.FillThreshold)
            {
                GRANNY_INC_INT_ACCUMULATOR(g_LocalPoseFrameStats.RefPoseFills);
            ...
            {
                GRANNY_INC_INT_ACCUMULATOR(g_LocalPoseFrameStats.Lerps);
            ...
            else
            {
                // Just set the transform to the reference transform.
                //
                // Note!  When we do this, invalidate the traversal id.  That allows us to
                // determine which bones have real data in them.  This isn't super ideal
                // in the LOD case, since there's some scalar percentage of the reference
                // pose blended in, but there's not much we can do about scalar->bool
                // mappings.
                GRANNY_INC_INT_ACCUMULATOR(g_LocalPoseFrameStats.RefPoseCopies);

                Local->Weight = 1.0f;
                Local->Count  = 1;
                Local->TraversalID = LocalPose.TraversalID - 1;
                Local->Transform = ReferenceSkeleton.Bones[SkeletonBoneIndex].LocalTransform;

                // todo: this drops through to the scale below, why?
            }

#if DEBUG
            quad OriginalOrientation;
            VectorEquals4(OriginalOrientation, Local->Transform.Orientation);
            real32 OriginalLength = VectorLength4(OriginalOrientation);
            OriginalLength = OriginalLength;        // Stops warnings.
#endif

            GRANNY_INC_INT_ACCUMULATOR(g_LocalPoseFrameStats.AccumScales);
            if (Local->Weight != 1.0f)
            {
                real32 const InverseTotalWeight = 1.0f / Local->Weight;
                VectorScale3(Local->Transform.Position, InverseTotalWeight);
                MatrixScale3x3((real32 *)Local->Transform.ScaleShear, InverseTotalWeight);
            }
            NormalizeWithTest4(Local->Transform.Orientation);

#if DEBUG
            real32 OrientationLength = VectorLength4(Local->Transform.Orientation);
        ...
        else
        {
            ...
            // note: already invalidated (by the test that led to this branch).  See the
            // comment above (marked "Note!") on why it's important to NOT mark TraveralID
            // as valid on this pass.  Local->TraversalID = LocalPose.TraversalID - 1;
                            ...
                            int32x const *SparseBoneArrayReverse)
{
    ...
    ALIGN16_STACK(real32, Temp, 16);
    ALIGN16_STACK(real32, Temp2, 16);
    ALIGN16_STACK(real32, InterResult, 16);

    MatrixIdentity4x4((real32*)&InterResult);
    while(BoneIndex != NoParentBone)
    {
        ...
        if ( SparseBoneArrayReverse != NULL )
        {
            ...
            CheckInt32Index(SparseBoneArrayReverse[BoneIndex], GetLocalPoseBoneCount(LocalPose), return);
            Local = GetLocalPoseTransform(LocalPose, SparseBoneArrayReverse[BoneIndex]);
        }
        else
        {
            // Direct mapping.
            CheckInt32Index(BoneIndex, GetLocalPoseBoneCount(LocalPose), return);
            Local = GetLocalPoseTransform(LocalPose, BoneIndex);
        }

        BuildCompositeTransform4x4(*Local, Temp);
        ColumnMatrixMultiply4x3Impl(Temp2, InterResult, Temp);
        MatrixEquals4x4((real32*)&InterResult, Temp2);
        BoneIndex = Skeleton.Bones[BoneIndex].ParentIndex;
    }

    if (Offset4x4)
    {
        ColumnMatrixMultiply4x3Impl(Temp2, InterResult, Offset4x4);
        MatrixEquals4x4(Result4x4, Temp2);
    }
    else
    {
        MatrixEquals4x4(Result4x4, InterResult);
    }
}
                    ...
                    int32x const *SparseBoneArrayReverse)
{
    ...
    GetWorldMatrixFromLocalPose(Skeleton, BoneIndex,
                                ...
                                NULL,
                                Temp,
                                SparseBoneArray, SparseBoneArrayReverse);
    MatrixInvert4x3(Result4x4, (real32 const *)Temp);

    if (Offset4x4)
    {
        ColumnMatrixMultiply4x3Impl(Temp,
                                    Result4x4,
                                    Offset4x4);
        MatrixEquals4x4(Result4x4, (real32 const *)Temp);
    }
}
                                   ...
                                   int32x const *SparseBoneArray)
{
    ...
    CheckCondition(GetLocalPoseBoneCount(CompositePose) == BoneCount, return);
    if ( Mask == NULL )
    {
...
void GRANNY
MaskedAdditiveBlend(local_pose*       Into,
                    local_pose const* Additive,
                    local_pose const* Base,
                    int32x            BoneStart,
                    int32x            BoneCount,
                    ...
                    real32            Amount)
{
    ...
    CheckPointerNotNull(Into,      return);
    CheckPointerNotNull(Additive,  return);
    CheckPointerNotNull(Base,      return);
    CheckCondition(BoneStart >= 0, return);
    CheckCondition(BoneCount >= 0, return);
    CheckCondition(GetLocalPoseBoneCount(*Into)     >= BoneStart + BoneCount, return);
    CheckCondition(GetLocalPoseBoneCount(*Additive) >= BoneStart + BoneCount, return);
    CheckCondition(GetLocalPoseBoneCount(*Base)     >= BoneStart + BoneCount, return);
    CheckBoundedReal32(0, Amount, 1, return);

    transform Id;
    MakeIdentity(Id);

    {for (int32 BoneIdx = BoneStart; BoneIdx < BoneStart + BoneCount; ++BoneIdx)
    {
        transform*       IntoXF = GetLocalPoseTransform(*Into,     BoneIdx);
        transform const* AddXF  = GetLocalPoseTransform(*Additive, BoneIdx);
        transform const* BaseXF = GetLocalPoseTransform(*Base,     BoneIdx);

        transform InvBase;
        BuildInverse(InvBase, *BaseXF);
        PostMultiplyBy(InvBase, *AddXF);

        real32 MaskWeight = 1.0f;
        if (ModelMask)
            MaskWeight = GetTrackMaskBoneWeight(*ModelMask, BoneIdx);

        transform Add;
        LinearBlendTransform(Add, Id, Amount * MaskWeight, InvBase);
        PostMultiplyBy(*IntoXF, Add);
    }}
}

void GRANNY
AdditiveBlend(local_pose*       Into,
              local_pose const* Additive,
              local_pose const* Base,
              int32x            BoneStart,
              int32x            BoneCount,
              real32            Amount)
{
    MaskedAdditiveBlend(Into, Additive, Base, BoneStart, BoneCount, 0, Amount);
}


static void
LinearBlendRangedNeighborhood(local_pose&       Into,
                              local_pose const& From,
                              local_pose const& To,
                              int32x            BoneStart,
                              int32x            BoneCount,
                              real32            Amount,
                              skeleton*         Neighborhood)
{
    ...
    CheckCondition(BoneCount >= 0, return);
    CheckCondition(GetLocalPoseBoneCount(Into) >= BoneStart + BoneCount, return);
    CheckCondition(GetLocalPoseBoneCount(From) >= BoneStart + BoneCount, return);
    CheckCondition(GetLocalPoseBoneCount(To)   >= BoneStart + BoneCount, return);
    CheckBoundedReal32(0, Amount, 1, return);

    if (Amount <= BlendEffectivelyZero)
    {
        CopyLocalPoseBoneCount(From, Into, BoneStart, BoneCount);
        return;
    }
    else if (Amount >= BlendEffectivelyOne)
    {
        CopyLocalPoseBoneCount(To, Into, BoneStart, BoneCount);
        return;
    }

    // Ok, we have real work to do.
    {for (int32x BoneIndex = BoneStart; BoneIndex < BoneStart + BoneCount; ++BoneIndex)
    {
        local_pose_transform& FromTransform = From.Transforms[BoneIndex];
        local_pose_transform& ToTransform   = To.Transforms[BoneIndex];

        transform BoneTransform;
        if (Neighborhood)
        {
            LinearBlendTransformNeighborhooded(BoneTransform,
                                               FromTransform.Transform, Amount,
                                               ToTransform.Transform,
                                               Neighborhood->Bones[BoneIndex].LocalTransform);
        }
        else
        {
            LinearBlendTransform(BoneTransform, FromTransform.Transform, Amount, ToTransform.Transform);
        }

        local_pose_transform& IntoTransform = Into.Transforms[BoneIndex];
        IntoTransform.Transform = BoneTransform;

        // Ape the behavior of ModulationCompositeLocalPose w.r.t. the ancillary fields.
        // Arguably, these should be a lerp.
        IntoTransform.Weight = FromTransform.Weight;
        IntoTransform.Count  = FromTransform.Count;
        IntoTransform.TraversalID = FromTransform.TraversalID;
    }}
}

void GRANNY
LinearBlendRanged(local_pose&       Into,
                  local_pose const& From,
                  local_pose const& To,
                  int32x            BoneStart,
                  int32x            BoneCount,
                  real32            Amount)
{
    ...
    LinearBlendRangedNeighborhood(Into, From, To, BoneStart, BoneCount, Amount, 0);
}


void GRANNY
LinearBlend(local_pose&       Into,
            local_pose const& From,
            local_pose const& To,
            real32            Amount)
{
    CheckCondition(GetLocalPoseBoneCount(From) == GetLocalPoseBoneCount(To), return);
    CheckCondition(GetLocalPoseBoneCount(Into) >= GetLocalPoseBoneCount(From), return);
    CheckBoundedReal32(0, Amount, 1, return);

    LinearBlendRangedNeighborhood(Into, From, To, 0, GetLocalPoseBoneCount(From), Amount, 0);
}


void GRANNY
LinearBlendNeighborhood(local_pose&       Into,
                        local_pose const& From,
                        local_pose const& To,
                        real32            Amount,
                        skeleton&         Neighborhood)
{
    ...
    LinearBlendRangedNeighborhood(Into, From, To, 0, GetLocalPoseBoneCount(From), Amount, &Neighborhood);
}
                              ...
                              int32x BoneCount)
{
    ...
    {for(int32x BoneIdx = FirstBone + BoneCount - 1; BoneIdx >= 0; --BoneIdx)
    {
        ...
        matrix_4x4 Local;
        ColumnMatrixMultiply4x3Impl((real32*)&Local,
...
{
    CheckCondition(Dst.BoneCount >= Src.BoneCount, return);
    CopyLocalPoseBoneCount(Src, Dst, 0, Src.BoneCount);
}

void GRANNY
CopyLocalPoseBoneCount(local_pose const &Src, local_pose &Dst,
                       int32x BoneStart,
                       int32x BoneCount)
{
    CheckCondition(BoneStart >= 0, return);
    CheckCondition(BoneCount >= 0, return);
    CheckCondition(Dst.BoneCount >= BoneStart + BoneCount, return);
    CheckCondition(Src.BoneCount >= BoneStart + BoneCount, return);

    if (&Src == &Dst)
    {
        // This is odd.  Should it be a log warning?  Certainly there's no work to be done...
        return;
    }

    Copy(sizeof(Src.Transforms[0]) * BoneCount,
         Src.Transforms + BoneStart,
         Dst.Transforms + BoneStart);
    Dst.FillThreshold = Src.FillThreshold;
    Dst.TraversalID   = Src.TraversalID;

    // Note that we do /not/ want to alter the NextPose field.  These poses could be
    // chained separately.
    // Dst.NextPose

    // Ensure that the destination TraversalID fields are consistent, even if the source
    // pose was smaller
    {for(int32x Idx = 0; Idx < Dst.BoneCount; ++Idx)
    {
        Dst.Transforms[Idx].TraversalID = Dst.TraversalID;
    }}
}
...
BEGIN_GRANNY_NAMESPACE;
void LocalPoseFrameStats()
{
    GRANNY_EMIT_INT_ACCUMULATOR("Granny/LocalPose/Ref. Pose Fills",  g_LocalPoseFrameStats.RefPoseFills);
    GRANNY_EMIT_INT_ACCUMULATOR("Granny/LocalPose/Ref. Pose Copies", g_LocalPoseFrameStats.RefPoseCopies);
    GRANNY_EMIT_INT_ACCUMULATOR("Granny/LocalPose/Lerps",            g_LocalPoseFrameStats.Lerps);
    GRANNY_EMIT_INT_ACCUMULATOR("Granny/LocalPose/AccumScales",      g_LocalPoseFrameStats.AccumScales);
}
END_GRANNY_NAMESPACE;
