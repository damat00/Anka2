...
static void scan_hashes( LZC lz, QUICKFIND * start, U32 which, U8 const* cur, U32 cur_len, U32 stop_len, U32* long_len, void const ** match, U32 * bit_pat, F32 * best_comp )
{
  ...
  do
  {
    ...
    if ( quick->addr[ 0 ] == cur[ 0 ] )
    {
      ...
      {
        cmp1 = if_start + quick->run_count + 1;
        cmp2 = cur + lz->run + 1;
        if ( cmp1 >= cmp2)
        {
          if_start = cur - 1;
                                      ...
                                      U32 max_offset )
{
  ...
  RR_UNUSED_VARIABLE(max_byte_value);
  RR_UNUSED_VARIABLE(uniq_byte_values);
                                 ...
                                 U32 max_offset )
{
  ...
  memset( l->quick2, 0, l->hash2_size * sizeof(void*) );

  l->quick3 = l->quick2 + l->hash2_size;
  memset( l->quick3, 0, l->hash3_size * sizeof(void*) );

  l->quick4 = l->quick3 + l->hash3_size;
  memset( l->quick4, 0, l->hash4_size * sizeof(void*) );

  l->buffer = ( QUICKFIND* ) ( l->quick4 + l->hash4_size );
  memset( l->buffer, 0, l->max_offs * sizeof( QUICKFIND ) );
...
RADDEFFUNC U32 LZ_compress( LZC l, ARITHBITS* ab, U8 const * input, U32 input_len )
{
  ...
  else
  {
    ...
    first_char_lit_size = Arith_combine_probabilities( lit_len_size , Arith_inv_probability( l->bytes[ ( (UINTa) input ) & ADDRESS_MASK ], input[ 0 ], l->max_bytes ) );
    ...
    if ( len >= 2 )
    {
      ...
      if ( len <= 5 )
      {
        ...
        for ( j = 1 ; j < len ; j++ )
        {
          ...
          Arith_adjust_probability( l->bytes[ ( (UINTa) &input[ j - 1 ] ) & ADDRESS_MASK ] , input[ j - 1 ], 1 );

          lits_size = Arith_combine_probabilities( lits_size, Arith_combine_probabilities( lit_len_size , Arith_inv_probability( l->bytes[ ( (UINTa) &input[ j ] ) & ADDRESS_MASK ], input[ j ], l->max_bytes ) ) );
        }

        // reset the probabilities
        for ( t = 1 ; t < j ; t++ )
        {
          Arith_adjust_probability( l->bytes[ ( (UINTa) &input[ t - 1 ] ) & ADDRESS_MASK ], input[ t - 1 ], -1 );
        }
        ...
        l->save_len = find_match( l,
                                  l->save_pos,
                                  input_len - 1,
                                  Arith_inv_probability_to_bits( Arith_combine_probabilities( lit_len_size, Arith_inv_probability( l->bytes[ ( (UINTa) l->save_pos ) & ADDRESS_MASK ], l->save_pos[ 0 ], l->max_bytes ) ) ),
                                  &l->save_match,
                                  &l->save_bit_pat );
    ...
    else
    {
     ...
     literal:
      ...
      // dump the byte
      if ( Arith_was_escaped( Arith_compress( l->bytes[ ( (UINTa) input ) & ADDRESS_MASK ], ab, *input ) ) )
      {
        // dump the escaped value
