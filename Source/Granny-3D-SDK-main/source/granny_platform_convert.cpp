// ========================================================================
// $File: //jeffr/granny_29/rt/granny_platform_convert.cpp $
// $DateTime: 2012/10/24 17:39:20 $
// $Change: 40020 $
// $Revision: #7 $
//
...
#if !PLATFORM_ANDROID
CompileAssert(OffsetFromType(data_type_definition, Name) == DataTypeNameOffset64);
CompileAssert(OffsetFromType(data_type_definition, ReferenceType) == DataTypeReferenceTypeOffset64);
#else
  // For some reason, android doesn't like these at file scope in pepper 18
  static void TestFn()
  {
...
enum sub_section_index
{
    ...
    sub_section_index_forceint = 0x7fffffff
};
...
struct conversion_state
{
    ...
    bool32 SourceIsLittleEndian;
    int32x DestPointerSize;
    int32x DestPointerSizeInBytes;
    bool32 DestIsLittleEndian;
...
FreeConversionState(conversion_state *OldState)
{
    if (OldState)
    {
        {for (int32x SecIdx = 0; SecIdx < OldState->NumFileSections; ++SecIdx)
        {
            {for (int32x SubIdx = 0; SubIdx < NumSubsections; ++SubIdx)
            {
                if (OldState->NewSections[SecIdx].Pointer[SubIdx])
                {
                    Deallocate(OldState->NewSections[SecIdx].Pointer[SubIdx]);
                    OldState->NewSections[SecIdx].Pointer[SubIdx] = 0;
                }
            }}
        }}
                          ...
                          conversion_state& ConversionState)
{
    ...
    bool const BytesReversed = (Bool32(PROCESSOR_LITTLE_ENDIAN) !=
                                Bool32(ConversionState.SourceIsLittleEndian));
                  ...
                  conversion_state& ConversionState)
{
    ...
    bool const BytesReversed = (Bool32(PROCESSOR_LITTLE_ENDIAN) !=
                                Bool32(ConversionState.DestIsLittleEndian));
    ...
    while (true)
    {
        ...
        if (DataType.Type == InlineMember)
        {
                                      ...
                                      &FixedUp))
            {
                ...
                FinalSize +=
                    (RealArraySize * ExtractDataTypeSizePlatform(FixedUp, PointerSizeInBits, ConversionState));
            }
            else
            {
                // This is really bad
        ...
        switch (DataType.Type)
        {
            ...
            case InlineMember:
            {
                                          ...
                                          ConversionState, &FixedUpType))
                {
                    ...
                    int32x  const SourceSize = ExtractDataTypeSizePlatform(FixedUpType, ConversionState.SourcePointerSize, ConversionState);
                    uint32x const DestSize  = ExtractDataTypeSizePlatform(FixedUpType, ConversionState.DestPointerSize, ConversionState);

                    int32x const ArrayWidth = DataType.ArrayWidth ? DataType.ArrayWidth : 1;
                    {for(int32x Idx = 0; Idx < ArrayWidth; ++Idx)
                    {
                                           ...
                                           ConversionState, &Ignored))
                        {
                            ...
                            Assert(SectionSizeRef == OldSectionSize + (DestSize*(Idx+1)));
                        }
    ...
    while (true)
    {
        ...
        switch (DataType.Type)
        {
            case InlineMember:
            {
                ...
                temp_grn_reference NewInlineTypeReference;
                if (!ObjectAlreadyCopied(ConversionState, ReferencedType, &NewInlineTypeReference))
                {
            ...
            case ReferenceToArrayMember:
            {
                                          ...
                                          &ReferencedArray))
                {
                    ...
                    temp_grn_reference ArrayStartRef = { 0 };
            ...
            case ArrayOfReferencesMember:
            {
                                          ...
                                          &ReferencedArray))
                {
                    ...
                    // We do need to align the destination pointer
                    {
                        grn_section* SectionArray = GetGRNSectionArray(*ConversionState.File->Header);
                        ArraySection.Size[IndexAny] =
                            AlignSize(ArraySection.Size[IndexAny],
                                      SectionArray[ReferencedArray.SectionIndex].InternalAlignment);
                    }
            ...
            case ReferenceToVariantArrayMember:
            {
                ...
                int32x const VarTypeOffset = ObjReference.Offset + ObjOffset;
                int32x const CountOffset   = VarTypeOffset + ConversionState.SourcePointerSizeInBytes;
        ...
        // Step the ObjOffset:
        // Note!  We know we can get away with this, the inline member type adjusts the objoffset and destobjoffect since the inline
                              ...
                              grn_mixed_marshalling_fixup **SectionMarshalls)
{
    ...
    {for(uint32x Index = 0; Index < NumSections; Index++)
    {
        ...
        {
            // Zero that memory to be sure that the compression winds up exactly the same
            // every time this is run
            Zero(TotalSize + OodlePadding, Section.Flattened);
        }
                   ...
                   grn_mixed_marshalling_fixup **SectionMarshalls)
{
    ...
    {
        Initialize(ConversionState->FixupRemaps);
    ...
    {for(int32x SecIndex = 0; SecIndex < ConversionState->NumFileSections; ++SecIndex)
    {
        ...
        {for(int32x Sub = 0; Sub < NumSubsections; ++Sub)
        {
            if (Section.Size[Sub])
            {
                Section.Pointer[Sub] = (uint8*)AllocateSize(Section.Size[Sub], AllocationFileData);
    ...
    ConvertFile->IsByteReversed = Bool32(PROCESSOR_LITTLE_ENDIAN) != Bool32(ConversionState->DestIsLittleEndian);
    ...
    {for(int32x SecIndex = 0; SecIndex < ConversionState->NumFileSections; ++SecIndex)
    {
        ...
        ConvertFile->Marshalled[SecIndex] = Bool32(ConvertFile->IsByteReversed);
    }}
