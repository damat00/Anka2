// ========================================================================
// $File: //jeffr/granny_29/rt/granny_string_formatting.cpp $
                      ...
                      uint32 Precision, char const *DigitTable)
{
    ...
    else if(IsReal64InfinityPattern(Real64))
    {
        float_conversion_64 cu;
        cu.Float = Real64;
        if ((cu.Integer & (1ULL << 63)) == 0)
        {
            return(ConvertToStringVar(OutputStringLength, OutputString, "(infinity)"));
        }
        else
        {
            return(ConvertToStringVar(OutputStringLength, OutputString, "(-infinity)"));
    ...
    else if(IsReal64QuietNanPattern(Real64))
    {
        return(ConvertToStringVar(OutputStringLength, OutputString, "(qnan)"));
    }
    else if(IsReal64SignallingNanPattern(Real64))
    {
        return(ConvertToStringVar(OutputStringLength, OutputString, "(snan)"));
    }
...
#if !COMPILER_GCC && !COMPILER_RVCT
                        ArgumentListArgument(List, intaddrx) = SpaceUsed;
#else
                        // Is this still true?
                        // Asnwer: apparently yes.
