// ========================================================================
// $File: //jeffr/granny_29/rt/granny_tangent_frame.cpp $
...
LockSeamTangents(mesh_builder& Builder)
{
    ...
    tangent_frame *TempFrames = AllocateArray(VertexCount, tangent_frame, AllocationTemporary);
...
ComputeTriangleTangents(mesh_builder& Builder)
{
         ...
         ++TriangleIndex)
    {
		...
		{
			VectorZero3(Tangent.U);
			VectorZero3(Tangent.V);
		}
    }}
...
#endif
{
    ...
    {
        // If they're different, I'm going to be paranoid and check that they weren't only
        ...
        {
            // I assumed that if they weren't bitwise-correct, that I didn't need to check
            // to a tolerance, because the only thing ComputeVertexTangents does is copy
            // vertex data around, never does computation with it.  So check this is
            // correct.
            ...
            // Note that for example these verts might still differ in the second texture
            // coord, or by a colour or something. So just because we got here doesn't
            // necessarily mean they match.
        }
#endif
...
ComputeVertexTangents(mesh_builder& Builder)
{
         ...
         ++TriangleIndex)
    {
        ...
        {
            // Look for one of the vertices that share this position, and find one that
            // matches the UV, etc coords (the VerticesAreEqual test) and that has the
            // same tangent-space winding (or none exists for that vertex yet).
            ...
            // Note that we are depending on an implementation detail of the stack
            // allocator here, which is that stacks do NOT change the address of any units
            // when new units are pushed onto them.  Throwing this assert in here mostly
            // to detect when the implementation of the stack changes substantially.
            ...
            if (Builder.AllowTangentVertSplits &&
                (TangentsAreOppositeWinding(*VertexTangent, TriangleTangent) ||
                                                      ...
                                                      Builder.TangentMergingMinCosine)))
            {
                ...
                {
                    // This is a really unfortunate mess of implementation details.  Note
                    // that we have to increase the size of all stacks marked with "per
                    // vertex" in the mesh_builder structure.  Assert that the indices
                    // wind up the way we expect...
