// ========================================================================
// $File: //jeffr/granny_29/rt/granny_track_group_builder.cpp $
...
{
    track_group_builder *Builder = Allocate(track_group_builder, AllocationBuilder);
    if(Builder)
    {
...
EndTrackGroup(track_group_builder *Builder)
{
    ...
    if(Builder)
    {
        ...
        TrackGroup = EndTrackGroupInPlace(Builder, AllocateSize(TotalSize, AllocationBuilder));
    }
    ...
    if(Builder)
    {
        ...
        if(EndAggrPlacement(Allocator, Memory))
        {
            ...
            {for ( int32x LODErrorNum = 0; LODErrorNum < TrackGroup->TransformLODErrorCount; LODErrorNum++ )
            {
                ...
                TrackGroup->TransformLODErrors[LODErrorNum] = GetReal32VeryLarge();
            }}
...
{
    text_track_builder *TrackBuilder = Add(&Builder.TextTrackTree, Name);
    if(TrackBuilder)
    {
...
EndTextTrack(track_group_builder &Builder)
{
    if (Builder.LastTextTrack)
    {
        if (Builder.LastTextTrack->EntryCount != 0)
            ++Builder.TextTrackCount;
...
ResortTrackGroup(track_group &Group)
{
         ...
         ++TrackIndex)
    {
        ...
        if(TrackBuilder)
        {
            ...
            if (Group.TransformLODErrors)
                TrackBuilder->DataLOD = Group.TransformLODErrors[TrackIndex];
        }
    }}

    {
        int32x TrackIndex = 0;
        {for(transform_track_builder* SourceBuilder = First(&TransformTrackTree);
             SourceBuilder;
                 ...
                 ++TrackIndex)
        {
            ...
            if (Group.TransformLODErrors)
                Group.TransformLODErrors[TrackIndex] = SourceBuilder->DataLOD;
        }}
    }
...
AllocateLODErrorSpace(track_group &Group)
{
    if ( Group.TransformLODErrorCount == 0 )
    {
        ...
        Group.TransformLODErrors = AllocateArray ( Group.TransformLODErrorCount, real32, AllocationBuilder );
    }
}
...
{
    SetAllLODErrorSpace ( Group, GetReal32VeryLarge() );
}
