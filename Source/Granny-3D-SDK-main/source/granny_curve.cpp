// ========================================================================
// $File: //jeffr/granny_29/rt/granny_curve.cpp $
                            ...
                            void *Memory, bool AllocateCurveMemory)
{
    ...
    if ( EndAggrPlacement ( Allocator, Memory ) )
    {
        ...
        // We need this to stay valid after setting it below in the KnotCount == 0 case...
        static real32 FakeKnotArray = 0.0f;
...
EndCurve(curve_builder *Builder)
{
    ...
    void *CurveMemory = AllocateSize ( TotalSize, AllocationBuilder );
...
{
    uint32x TotalSize = sizeof ( curve2 );
    int32x Dimension = 0;
...
{
    uint32x NumberOfBytes = sizeof (curve_data_da_k32f_c32f);
                   ...
                   bool RescaleResults)
{
    ...
    if (RescaleResults)
    {
        ...
        real32 MinDot =  GetReal32VeryLarge();
        real32 MaxDot = -GetReal32VeryLarge();
        {for(int Control = 0; Control < NumControls; ++Control)
        {
            real32 const* CurControl = ControlSrc + Control * ControlStrideInDwords;
...
#define EXTRACT_ONE_OVER_KNOT_SCALE OneOverKnotScale = GetReal32FromInt32(((uint32)CurveData.OneOverKnotScaleTrunc)<<16)
                                        ...
                                        real32 const *KnotSrc, real32 const *ControlSrc )
{
    ...
    CurveData.OneOverKnotScaleTrunc = (uint16)(GetInt32FromReal32(OneOverKnotScale)>>16);
    // And then re-extract it, coz it will have changed slightly.
    EXTRACT_ONE_OVER_KNOT_SCALE;
    Assert((real32)((real32)UInt16Maximum / KnotMax) >= OneOverKnotScale);
    {for ( int32x Count = 0; Count < KnotCount; Count++ )
    {
                                        ...
                                        real32 const *KnotSrc, real32 const *ControlSrc )
{
    ...
    // And then re-extract it, coz it will have changed slightly.
    EXTRACT_ONE_OVER_KNOT_SCALE;
    Assert((real32)((real32)UInt8Maximum / KnotMax) >= OneOverKnotScale);
    {for ( int32x Count = 0; Count < KnotCount; Count++ )
    {
...
CURVE_SCALE_OFFSET_SWIZZLE(MIXED_NAME) ( curve2 &Curve, int32x Dimension, real32 const *Scales, real32 const *Offsets, int32x const *Swizzles )
{
    ...
    {for ( int32x ControlNum = 0; ControlNum < CurveKnotCount; ControlNum++ )
    {
        ...
        {for ( int32x SrcComp = 0, DstComp = MissingComponentIndex;
               SrcComp < 3;
               SrcComp++ )
        {
            DstComp++;
