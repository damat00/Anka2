// ========================================================================
// $File: //jeffr/granny_29/rt/granny_compress_curve.cpp $
...
CompressCurve(bspline_solver &Solver,
              ...
              bool *CurveAcheivedTolerance)
{
    CheckCondition(Dimension < MaximumBSplineDimension, return 0);
    CheckCondition(CompressionParamsAreValid(Params, SolverFlags), return 0);
...
#if DEBUG_COMPRESSOR
    {
        ...
        real32 *ResultKnots    = AllocateArray(ResultKnotCount, real32, AllocationTemporary);
        real32 *ResultControls = AllocateArray(ResultKnotCount * Dimension, real32, AllocationTemporary);
    ...
    if(Params.AllowDegreeReduction)
    {
        ...
        if (!KnotsAreReducible(Dimension, FrameCount,
                               0,
                               Samples,
                               Params.IdentityVector, Params.ErrorTolerance,
                               DiffMode, ResultDegree, ResultKnotCount))
        {
            // We still want to check one case here.  In certain cases, if the values are
            // at or beyond float accuracy, the knots may come back different, even though
            // the underlying samples are identical.  In that case, we want to encode as a
            // constant, no matter what.
            bool AllIdentical = true;
            for (int Frame = 1; Frame < FrameCount && AllIdentical; ++Frame)
            {
                real32 const* BaseFrame = Samples;
                real32 const* ThisFrame = Samples + (Frame * Dimension);

                if (!Compare(sizeof(real32)*Dimension, BaseFrame, ThisFrame))
                    AllIdentical = false;
            }
