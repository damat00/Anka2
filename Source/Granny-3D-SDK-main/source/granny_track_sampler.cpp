// ========================================================================
// $File: //jeffr/granny_29/rt/granny_track_sampler.cpp $
...
void GRANNY
SetContextFrameIndex(sample_context& Context,
                     real32 Clock,
                     real32 Duration,
                     real32 TimeStep)
{
    if (Clock != Duration)
    {
        Context.FrameIndex = TruncateReal32ToInt32(Clock / TimeStep);
    }
    else
    {
        Context.FrameIndex = RoundReal32ToInt32(Duration / TimeStep);
    }
}
              ...
              bool CurveIsNormalized)
{
    ...
    else if (CurveIsKeyframed(Curve))
    {
        CurveExtractKnotValue ( Curve, Context.FrameIndex, Result, IdentityVector );
    }
    else
    {
...
#define BuildIdentity                                           \
    ...
    } typedef int Require_SemiColon_
...
#define BuildTransform                                      \
    {                                                       \
        BWP_Dispatch(&Transform, ParentMatrix, WorldResult); \
    } typedef int Require_SemiColon_
                    ...
                    transform &Result)
{
    ...
    {
        VectorIdentityQuaternion(Result.Orientation);
    }
#if BOUND_TRANSFORM_TRACK_HAS_LOD
...
SampleTrackUUULocalAtTime0 ( transform_track const *SourceTrack, transform &Result )
{
    ...
    if ( !CurveIsIdentity ( SourceTrack->OrientationCurve ) )
    {
        ...
        // On older (or newer) real32 curves that are fit without the ForceEndpoint flag,
        // it's possible that the first knot will be off the hypersphere, resulting in a
        // quaternion that is non-normalized.  Ensure that the returned transform has unit
        // length to allow consistency checks of the orientation.
                   ...
                   real32 *ResultOrientation)
{
    ...
    {
        VectorIdentityQuaternion(ResultOrientation);
    }
}
...
TrackSampler(SampleTrackIIU)
{
    ...
    if ( CurveIsKeyframed ( SourceTrack->ScaleShearCurve ) )
    {
        // Just sample it directly.
        transform Transform;
        VectorZero3(Transform.Position);
        VectorIdentityQuaternion(Transform.Orientation);
        ...
        // Since this has scale shear, we know where it's going
        BuildFullTransform;
    }
