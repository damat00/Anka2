// ========================================================================
// $File: //jeffr/granny_29/rt/granny_image_operations.cpp $
...
#define ceil Ceiling
#define floor Floor
#define radmalloc(mallocsize) AllocateSize(mallocsize, AllocationUnknown)
...
//#define TEST
    ...
    for(i = 0; i < (s32)rs->destheight; ++i)
    {
      ...
      u32 val;
      if (weighta > 0.0f)
        val=(weighta>=254.5f)?(255U<<24):((RoundReal32ToInt32(weighta))<<24);
      else
        val=0;
      if (weightr>0.0f)
        val|=(weightr>=254.5f)?(255<<16):((RoundReal32ToInt32(weightr))<<16);
      if (weightg>0.0f)
        val|=(weightg>=254.5f)?(255<<8):((RoundReal32ToInt32(weightg))<<8);
      if (weightb>0.0f)
        val|=(weightb>=254.5f)?255:((RoundReal32ToInt32(weightb)));
