// ========================================================================
// $File: //jeffr/granny_29/rt/granny_s3tc.cpp $
...
static unsigned int findalpha4x4(unsigned short* output,
                                 ...
                                 unsigned int ip,
                                 bool Force8LevelAlpha);

static bool
MakePaddedImage(void const* original,
                int orig_stride,
                int orig_width,
                int orig_height,
                void*& padded,
                ...
                int& padded_height)
{
    Assert(orig_width < 4 || orig_height < 4 || orig_stride < 0);  // Otherwise, we shouldn't be here...
    ...
    uint8* PaddedImage = AllocateArray(padded_width * padded_height * 4, uint8, AllocationTemporary);
    padded = PaddedImage;

    int abs_stride = orig_stride < 0 ? -orig_stride : orig_stride;
    ...
    for (int y = 0; y < padded_height; ++y)
    {
        ...
        for (int x = 0; x < padded_stride; x += abs_stride)
        {
            int copy_bytes = padded_stride - x;
            Assert(copy_bytes > 0);
            if (copy_bytes > abs_stride)
                copy_bytes = abs_stride;
...
unsigned int GRANNY
to_S3TC1(void* output,const void* input, int instride, int width, int height)
{
    Assert(width > 0 && height > 0);

    if (width < 4 || height < 4 || instride < 0)
    {
        void* padded;
        int pad_stride, pad_width, pad_height;
...
unsigned int GRANNY
to_S3TC1a(void* output,const void* input, int instride, int width, int height)
{
    Assert(width >= 0);
    Assert(height >= 0);
...
unsigned int GRANNY
to_S3TC2or3(void* output,const void* input, int instride, int width, int height)
{
    Assert(width >= 0);
    Assert(height >= 0);
...
static unsigned int
S3TC4or5_Driver(void* output,
                const void* input,
                int instride,
                int width,
                int height,
                bool Force8LevelAlpha /* s3tc5 only */)
{
    Assert(width >= 0);
    Assert(height >= 0);
    ...
    if (width < 4 || height < 4 || instride < 0)
    {
        ...
        if (MakePaddedImage(input, instride, width, height,
                            padded, pad_stride, pad_width, pad_height))
        {
            unsigned int err = S3TC4or5_Driver(output, padded, pad_stride, pad_width, pad_height, Force8LevelAlpha);
            Deallocate(padded);
    ...
    for(y=(unsigned int)(height/4);y;y--)
    {
        for(x=(unsigned int)iw;x;x--)
        {
            err+=findalpha4x4(o,((unsigned char*)i)+3,ip*4,Force8LevelAlpha);
            o+=4;
            err+=findcolor4x4(o,i,ip);
            o+=4;
            i+=4;
            ...
            int height)
{
    ...
    return S3TC4or5_Driver(output, input, instride, width, height, false);
}

unsigned int GRANNY
to_S3TC5(void* output,
         const void* input,
         int instride,
         int width,
         int height)
{
    ...
    return S3TC4or5_Driver(output, input, instride, width, height, true);
}
                  ...
                  unsigned int* type)
{
    ...
    for(y=(unsigned int)(height/4);y;y--)
    {
        for(x=(unsigned int)iw;x;x--)
        {
            aerr4+=findalpha4x4(o,((unsigned char*)i)+3,ip*4,false);
...
//=========================================================================
// Utility functions to help out on big endian platforms
//   On the 360 and other big-endian platforms, we need to reverse values on read and
//   write before writing it out to the pixel buffer.
//   =========================================================================

unsigned int s3tc_32bitValue(unsigned short* v)
{
#if PROCESSOR_BIG_ENDIAN
    union { unsigned short rev[2]; uint32 val; } u;
    u.rev[0] = Reverse16_16(v[1]);
    u.rev[1] = Reverse16_16(v[0]);
    CompileAssert(sizeof(u) == sizeof(uint32));

    return u.val;
#else
    union { unsigned short* in; uint32* out; } u;
    u.in = v;
    return *u.out;
#endif
}

unsigned short s3tc_16bitValue(unsigned short* v)
{
#if PROCESSOR_BIG_ENDIAN
    return Reverse16_16(*v);
#else
    return *v;
#endif
}

unsigned int s3tc_OutputValue(unsigned int v)
{
#if PROCESSOR_BIG_ENDIAN
    return Reverse32(v);
#else
...
void GRANNY
from_S3TC1(unsigned int* outbuf,
           unsigned short* s3buf,
           int width,
           int height )
{
    Assert(width >= 0);
    ...
    for(y=(height/4);y;y--)
    {
        for(x=iw;x;x--)
        {
            ...
            c[0] = s3tc_16bitValue(s3buf + 0);
            c[1] = s3tc_16bitValue(s3buf + 1);
            bits = s3tc_32bitValue(s3buf + 2);
            ...
            // On the 360 and other big-endian platforms, we need to reverse the int
            // before writing it out to the pixel buffer.

            // do first row
            o[0]=s3tc_OutputValue(c[bits&3]); bits>>=2;
            o[1]=s3tc_OutputValue(c[bits&3]); bits>>=2;
            o[2]=s3tc_OutputValue(c[bits&3]); bits>>=2;
            o[3]=s3tc_OutputValue(c[bits&3]); bits>>=2;
            o+=width;
              ...
              int height)
{
    ...
    for(y=(height/4);y;y--)
    {
        ...
        {
            unsigned int c[4];
            unsigned int bits,alpha,alpha2;

            alpha  = s3tc_32bitValue(s3buf + 0);
            alpha2 = s3tc_32bitValue(s3buf + 2);

            c[0] = s3tc_16bitValue(s3buf + 4);
            c[1] = s3tc_16bitValue(s3buf + 5);

            bits = s3tc_32bitValue(s3buf + 6);
            ...
            // first row
            o[0] = s3tc_OutputValue(c[bits&3]|alpha4to8[alpha&15]); bits>>=2; alpha>>=4;
            o[1] = s3tc_OutputValue(c[bits&3]|alpha4to8[alpha&15]); bits>>=2; alpha>>=4;
            o[2] = s3tc_OutputValue(c[bits&3]|alpha4to8[alpha&15]); bits>>=2; alpha>>=4;
            o[3] = s3tc_OutputValue(c[bits&3]|alpha4to8[alpha&15]); bits>>=2; alpha>>=4;
            o+=width;
            ...
            o[1] = s3tc_OutputValue(c[bits&3]|alpha4to8[alpha&15]); bits>>=2; alpha>>=4;
            o[2] = s3tc_OutputValue(c[bits&3]|alpha4to8[alpha&15]); bits>>=2; alpha>>=4;
            o[3] = s3tc_OutputValue(c[bits&3]|alpha4to8[alpha]);    bits>>=2;
            o+=width;

            // third row
            o[0] = s3tc_OutputValue(c[bits&3]|alpha4to8[alpha2&15]); bits>>=2; alpha2>>=4;
            o[1] = s3tc_OutputValue(c[bits&3]|alpha4to8[alpha2&15]); bits>>=2; alpha2>>=4;
            o[2] = s3tc_OutputValue(c[bits&3]|alpha4to8[alpha2&15]); bits>>=2; alpha2>>=4;
            o[3] = s3tc_OutputValue(c[bits&3]|alpha4to8[alpha2&15]); bits>>=2; alpha2>>=4;
            o+=width;
            ...
            o[1] = s3tc_OutputValue(c[bits&3]|alpha4to8[alpha2&15]); bits>>=2; alpha2>>=4;
            o[2] = s3tc_OutputValue(c[bits&3]|alpha4to8[alpha2&15]); bits>>=2; alpha2>>=4;
            o[3] = s3tc_OutputValue(c[bits&3]|alpha4to8[alpha2]);
              ...
              int height )
{
    ...
    for(y=(height/4);y;y--)
    {
        for(x=iw;x;x--)
        {
            ...
            alpha  = s3tc_32bitValue(s3buf + 1);
            alpha2 = s3tc_16bitValue(s3buf + 3);
            ...
            // first row
            o[0] = s3tc_OutputValue(c[bits&3]|a[alpha&7]); bits>>=2; alpha>>=3;
            o[1] = s3tc_OutputValue(c[bits&3]|a[alpha&7]); bits>>=2; alpha>>=3;
            o[2] = s3tc_OutputValue(c[bits&3]|a[alpha&7]); bits>>=2; alpha>>=3;
            o[3] = s3tc_OutputValue(c[bits&3]|a[alpha&7]); bits>>=2; alpha>>=3;
            o+=width;
            ...
            o[1] = s3tc_OutputValue(c[bits&3]|a[alpha&7]); bits>>=2; alpha>>=3;
            o[2] = s3tc_OutputValue(c[bits&3]|a[alpha&7]); bits>>=2; alpha>>=3;
            o[3] = s3tc_OutputValue(c[bits&3]|a[alpha&7]); bits>>=2;

            // read the next 16 bits of alpha bits (48 bits total)
            alpha=(alpha>>3)|(alpha2<<8);
            o+=width;
            ...
            o[1] = s3tc_OutputValue(c[bits&3]|a[alpha&7]); bits>>=2; alpha>>=3;
            o[2] = s3tc_OutputValue(c[bits&3]|a[alpha&7]); bits>>=2; alpha>>=3;
            o[3] = s3tc_OutputValue(c[bits&3]|a[alpha]);
...
static void findbest(unsigned char* unique,unsigned int* counts,unsigned int tot,unsigned int* col, unsigned int max)
{
    ...
    for(i=0;i<SEARCHRANGE;i++)
    {
        for(j=i+1;j<SEARCHRANGE;j++)
        {
            ...
            Assert(col[i]*2+col[j] + 1 < 256);
            Assert(col[j]*2+col[i] + 1 < 256);
...
unsigned int find3from012(int c0, int c1, int c2, int maxVal)
{
    int c3 = (3 * c2 - 1 - c0) / 2;
    ...
    if (c3 >= 0 && (COLOR_1(c0, c3) == c1 && COLOR_2(c0, c3) == c2))
        ...
        return c3-1;
    else if (c3+1 >= 0 && c3+1 <= maxVal && (COLOR_1(c0, c3+1) == c1 && COLOR_2(c0, c3+1) == c2))
        return c3+1;
...
unsigned int find0from123(int c1, int c2, int c3, int maxVal)
{
    int c0 = (3 * c1 - 1 - c3) / 2;
    ...
    if (c0 >= 0 && (COLOR_1(c0, c3) == c1 && COLOR_2(c0, c3) == c2))
        ...
        return c0-1;
    else if (c0+1 >= 0 && c0+1 <= maxVal && (COLOR_1(c0+1, c3) == c1 && COLOR_2(c0+1, c3) == c2))
        return c0+1;
...
#define VALID_COLORS() \
    ...
    if (tot == 3)
    {
        ...
        for (i=0; i < 3*3 && besterr != 0; i++)
        {
            ...
            // the 4 possibilities
            // *1, *2 still valid
            r2 = find3from012(r1, ro, r2, 0x1f);
            g2 = find3from012(g1, go, g2, 0x3f);
            b2 = find3from012(b1, bo, b2, 0x1f);
            if (!((r1>r2) || ((r1==r2) && (g1>g2)) || ((r1==r2) && (g1==g2) && (b1>b2))))
            {
            ...
            g2 = (unique[second_color]>> 8)&0xff;
            b2 = (unique[second_color]>> 0)&0xff;
            r1 = find0from123(r1, ro, r2, 0x1f);
            g1 = find0from123(g1, go, g2, 0x3f);
            b1 = find0from123(b1, bo, b2, 0x1f);
            if (!((r1>r2) || ((r1==r2) && (g1>g2)) || ((r1==r2) && (g1==g2) && (b1>b2))))
            {
                if (r2 < 0x1f && g2 < 0x3f && b2 < 0x1f)
                {
                                 ...
                                 bool Force8LevelAlpha)
{
    ...
    if (tot==2)
    {
        ...
        for(p=0;p<16;p++)
        {
            unsigned int di1=Quick8Abs((unsigned int)unique[0]-(unsigned int)invert[p]);
            unsigned int di2=Quick8Abs((unsigned int)unique[1]-(unsigned int)invert[p]);
            if (di1<=di2)
            {
                tot=(tot<<3);
            }
    ...
    for(a1=amin;a1<=atop;a1++)
    {
        ...
        do
        {
            ...
            // Make sure that we get through this loop at least once, in case we're
            // forcing 8-level alpha.
            if (c[7] > 255)
            {
                if (a2 > 1)
                    break;
