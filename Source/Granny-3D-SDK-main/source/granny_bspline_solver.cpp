// ========================================================================
// $File: //jeffr/granny_29/rt/granny_bspline_solver.cpp $
...
//    }
    ...
    CheckGreaterEqual0(NumNewKnots, return 0);
...
static real32
FirstDerivative(int32x Dimension, real32 *Samples, int32x SampleIndex, bool AsQuaternions)
{
    real32 ResultA = 0;
    real32 ResultB = 0;
    ...
    if (AsQuaternions)
    {
        ResultA = Square(1.0f - InnerProductN(Dimension, &Samples[Dimension*SampleIndex], &Samples[Dimension*(SampleIndex-1)]));
        ResultB = Square(1.0f - InnerProductN(Dimension, &Samples[Dimension*SampleIndex], &Samples[Dimension*(SampleIndex+1)]));
    }
    else
    {
        {for(int32x Element = 0; Element < Dimension; ++Element)
        {
...
static real32
SecondDerivative(int32x Dimension, real32 *Samples, int32x SampleIndex, bool AsQuaternions)
{
    real32 Result = 0;

    if (AsQuaternions)
    {
        real32 Pre  = InnerProductN(Dimension, &Samples[Dimension*SampleIndex], &Samples[Dimension*(SampleIndex-1)]);
        real32 Post = InnerProductN(Dimension, &Samples[Dimension*SampleIndex], &Samples[Dimension*(SampleIndex+1)]);

        Result = Square(Post - Pre);
    }
    else
    {
        {for(int32x Element = 0;
                    ...
                    real32 SecondDerivativeThreshold)
{
    ...
    bool AsQuaternions = (Flags & BSplineSolverEvaluateAsQuaternions) != 0;
    {for(int32x SampleIndex = 1;
         SampleIndex < (SampleCount - 1);
         ++SampleIndex)
    {
        real32 SD = SecondDerivative(Dimension, Samples, SampleIndex, AsQuaternions);
        real32 FD = FirstDerivative(Dimension, Samples, SampleIndex, AsQuaternions);
                    ...
                    int32x *CurveSizeInBytes)
{
    ...
    real32 *TransformedSamples = AllocateArray(Dimension * SampleCount, real32, AllocationTemporary);
    {
...
{
    CheckBoundedReal32(0.0f, Degrees, GetReal32VeryLarge(), return 0.0f);
...
{
    CheckBoundedReal32(0.0f, Tolerance, 2.0f, return 180.0f);
