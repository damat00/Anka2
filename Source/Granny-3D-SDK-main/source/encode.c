...
#include <string.h>
#if defined(__RADLINUX__) || defined(__RADMAC__)
  #include <stdlib.h>
#endif

// gets the bitlevel of a int
#define GET_BIT_LEVEL( val )  ( (U32)getbitlevelvar( val ) )
...
static void decode_low( ARITHBITS* ab, VARBITS* vb, S16 * outp, S32 pixel_pitch, U32 enc_width, U32 enc_height, void * temp )
{
  ...
  *outp++ = (S16) prev;
  ...
  for( w = ( enc_width - 1 ) ; w-- ; )
  {
    UINTa cur = Arith_decompress( a, ab );
