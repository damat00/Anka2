...
// ========================================================================
// $File: //jeffr/granny_29/rt/granny_s3tc.h $
...
EXPTYPE enum s3tc_texture_format
{
    ...
    s3tc_texture_format_forceint = 0x7fffffff
};
...
unsigned int to_S3TC4or5(void *output, const void* input,
                         int instride,
                         int width, int height);
unsigned int to_S3TC5(void *output, const void* input,
                      int instride,
                      int width, int height);
...
// Decompressors
void from_S3TC1(uint32* outbuf, uint16* s3buf,
                int width, int height);
void from_S3TC2or3(uint32* outbuf, uint16* s3buf,
                   int width, int height);
void from_S3TC4or5(uint32* outbuf, uint16* s3buf,
                   int width, int height);
