// ========================================================================
// $File: //jeffr/granny_29/rt/granny_variant_builder.cpp $
...
{
    variant_builder *Builder = Allocate(variant_builder, AllocationUnknown);
    if(Builder)
    {
...
void* GRANNY
EndVariant(variant_builder*       Builder,
           data_type_definition*& Type,
           void*&                 Object)
{
    void* RetPtr = 0;
    ...
    if(Builder)
    {
        ...
        RetPtr = EndAggrAlloc(Allocator, AllocationBuilder);

        EndVariantInPlace(Builder, TypeMemory, Type, ObjectMemory, Object);
    }

    Assert(RetPtr || (Type == 0 && Object == 0));
    return RetPtr;
}
...
void GRANNY
AddBoolArrayMember(variant_builder &Builder, char const *Name, int32x Width, bool32* Value)
{
    Name = MapString(*Builder.Strings, Name);
    data_type_definition Type = {Bool32Member, Name, 0, Width};
    AddMember(Builder, Type, Value);
}
