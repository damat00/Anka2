...
// ========================================================================
// $File: //jeffr/granny_29/rt/granny_math_inlines.h $
...
inline real32 ASin(real32 const Value)
{
    return((real32)IntrinsicASin(Value));
}
...
{
    // NB: This looks a bit odd, why are we setting Value to Min if it's already equal to
    // Min?  The reason is that we want -0.0 to be reset to 0.0 if we're using a 0 bound.
    if (Value <= Min)
    {
        Value = Min;
    }
...
inline int32x
ClampInt32(int32x Min, int32x Value, int32x Max)
{
    if(Value < Min)
        Value = Min;
    else if(Value > Max)
...
inline bool QuaternionBetweenVectors(real32* Quaternion,
                                     real32 const* Vec0,
                                     real32 const* Vec1)
{
    Assert(Quaternion);
    ...
    real32 Len0Squared = VectorLengthSquared3(Vec0);
    real32 Len1Squared = VectorLengthSquared3(Vec1);

    // From normalizeorzero3
    const real32 Epsilon = 0.00001f * 0.00001f;
    if (Len0Squared >= Epsilon && Len1Squared >= Epsilon)
    {
        // Trussssst me....
        VectorCrossProduct3(Quaternion, Vec0, Vec1);
        Quaternion[3] = InnerProduct3(Vec0, Vec1) + SquareRoot(Len0Squared * Len1Squared);
        Normalize4(Quaternion);
        return true;
    }
    else
    {
        VectorSet4(Quaternion, 0, 0, 0, 1);
        return false;
    }
}
...
inline real32
InnerProductUnitClamped3(real32 const *V0, real32 const *V1)
{
    return Clamp(-1, InnerProduct3(V0, V1), 1);
}
...
#if !PLATFORM_HAS_FAST_MATH_INLINES
