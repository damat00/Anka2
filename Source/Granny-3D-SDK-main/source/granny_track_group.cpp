// ========================================================================
// $File: //jeffr/granny_29/rt/granny_track_group.cpp $
                   ...
                   curve2 *Curve)
{
    ...
    if ( !BasisConversionRequiresCurveDecompressionConst ( Affine3, Linear3x3, NULL, AffineTolerance, LinearTolerance ) )
    {
        ...
        {
            Knots = AllocateArray ( KnotCount, real32, AllocationTemporary );
        }
        real32 *Controls = AllocateArray ( 3 * KnotCount, real32, AllocationTemporary );
    ...
    else
    {
        ...
        if ( !CurveIsKeyframed ( *Curve ) )
        {
            Knots = AllocateArray ( NumKnots, real32, AllocationTemporary );
        }
        real32 *Controls = AllocateArray ( NumKnots * Dimension, real32, AllocationTemporary );
        if ( Controls != NULL )
        {
...
#if DEBUG
        int32 KnotCount = CurveGetKnotCount ( *Curve );
        real32 *Knots = AllocateArray ( KnotCount, real32, AllocationTemporary );
        real32 *Controls = AllocateArray ( 4 * KnotCount, real32, AllocationTemporary );
                                   ...
                                   curve2 *Curve)
{
    ...
    if ( !BasisConversionRequiresCurveDecompressionConst ( GlobalZeroVector, Linear3x3, InverseLinear3x3, 1.0f, LinearTolerance ) )
    {
        ...
        int32 KnotCount = CurveGetKnotCount ( *Curve );
        real32 *Knots = AllocateArray ( KnotCount, real32, AllocationTemporary );
        real32 *Controls = AllocateArray ( 9 * KnotCount, real32, AllocationTemporary);
                     ...
                     real32 &ConstantDifference)
{
    ...
    {
        // Caller doesn't want to compare against an identity.
        IdentityDifference = GetReal32VeryLarge();
    }
}

bool GRANNY
KnotsAreReducible(int32x Dimension,
                  int32x KnotCount,
                  real32 const *Knots,
                ...
                int32x &Result)
{
    if(TrackGroup)
    {
        if(TrackGroup->Flags & TrackGroupIsSorted)
        {
            ...
            binary_searcher Search;
            InitializeSearch(Search, TrackGroup->TransformTrackCount);

            while (SearchRemaining(Search))
            {
                int32x Probe = GetSearchProbe(Search);

                int32x const Diff =
                    StringDifferenceOrCallback(TrackName, TrackGroup->TransformTracks[Probe].Name);
                ...
                if (Diff == 0)
                {
                    ...
                    UpdateGreaterThan(Search, Probe);
                else
                    UpdateLessThan(Search, Probe);
            }
        }
        else
        {
            // We have to do linear search
            {for(Result = 0; Result < TrackGroup->TransformTrackCount; ++Result)
            {
...
bool GRANNY
FindVectorTrack(track_group const *TrackGroup,
                ...
                int32x& Result)
{
    if(TrackGroup)
    {
        ...
        // We have to do linear search.  Note that we're changing the output parameter here...
        {for(Result = 0;
             Result < TrackGroup->VectorTrackCount;
             ++Result)
        {
            // Check the cheap part first...
            if (TrackGroup->VectorTracks[Result].TrackKey != TrackKey)
                continue;

            if (StringsAreEqualOrCallback(TrackName, TrackGroup->VectorTracks[Result].Name))
                return true;
        }}
    }
...
bool GRANNY
FindTextTrackByName(track_group const* TrackGroup,
                    char const* TrackName,
                    int32x& TextTrackIndex)
{
    if (TrackGroup && TrackName)
    {
        // Text tracks are not sorted (yet?)

        // We have to do linear search
        {for(int32x Result = 0;
             Result < TrackGroup->TextTrackCount;
             ++Result)
        {
            if (StringsAreEqualLowercaseOrCallback(TrackName, TrackGroup->TextTracks[Result].Name))
            {
                TextTrackIndex = Result;
                return true;
            }
        }}
    }
