...
// ========================================================================
// $File: //jeffr/granny_29/rt/granny_control.h $
...
struct control
{
    ...
    enum
    {
        ...
        KillClockIsLocal      = 0x40,
        EaseClockIsLocal      = 0x80
    };
    uint32x Flags;
    ...
    // When we die, if KillOnceComplete flag is set.  Note that if KillClockIsLocal is
    // set, we need to adjust this when the speed of the control is changed
    real32 KillClock;
...
EXPAPI GS_READ bool   GetControlLocalCompletion(control const &Control);

EXPAPI GS_PARAM void  SetControlCompletionCheckFlag(control &Control, bool CheckComplete);
EXPAPI GS_PARAM void  SetControlLocalCompletion(control &Control, bool CompleteIsLocal);

EXPAPI GS_READ bool ControlHasEffect(control &Control);
...
bool ControlIsRetargeted(control const& Control);
bool ControlIsMirrored(control const& Control);
...
EXPAPI GS_MODIFY bool SetControlModelMask(control const &Control,
                                          model_instance const* ModelInstance,
                                          track_mask* TrackMask);
EXPAPI GS_MODIFY bool SetControlTrackMask(control const &Control,
                                          animation const* Animation,
                                          int32x TrackGroupIndex,
                                          track_mask* TrackMask);
EXPAPI GS_MODIFY bool SetSPUControlTrackMask(control const &Control,
...
EXPAPI GS_READ track_mask const* GetControlTrackGroupTrackMask(control const &Control,
                                                               animation const* Animation,
                                                               int32x TrackGroupIndex);
EXPAPI GS_READ track_mask const* GetSPUControlTrackGroupTrackMask(control const &Control,
                                                                  spu_animation const* Animation,
                                                                  int32x TrackGroupIndex);
...
EXPAPI GS_READ real32 GetControlCycleDuration(control const &Control);
...
EXPAPI GS_READ  bool GetControlLocalEase(control const& Control);
EXPAPI GS_PARAM void SetControlLocalEase(control& Control, bool EaseIsLocal);
...
EXPAPI GS_PARAM real32 EaseControlIn(control &Control, real32 Duration, bool FromCurrent);
...
EXPAPI GS_PARAM bool GetControlForceClampedLooping(control &Control);
