...
// Control how many worker threads are started.  For the PS3, only 1 and 2 are valid
// values.
...
//-------------------------------------------------------------------------------------
char const *ModelFileName           = "/app_home/Media/test.gr2";
...
struct mesh
{
    ...
    vector<texture*> TextureReferences;
...
//-------------------------------------------------------------------------------------
#include <pthread.h>
void* ThreadedUpdate(void* arg)
{
    granny_int32x MyStart = 0;
    granny_int32x MyEnd   = (granny_int32x)arg;

    for (granny_int32x Idx = MyStart; Idx < MyEnd; ++Idx)
    {
...
{
    assert(numThreads > 0 && numThreads <= 2);

    g_scene.CurrentTime = CurrentTime;

    granny_int32x MyStart = 0;
    granny_int32x MyEnd = (granny_int32x)g_scene.Models.size();
    ...
    if (numThreads == 2)
    {
        granny_int32x OtherEnd = MyEnd/2;
        MyStart = OtherEnd;

        GrannySetAllowGlobalStateChanges(false);
        pthread_create(&ThreadId, NULL, ThreadedUpdate, (void*)OtherEnd);
    }
