                     ...
                     granny_memory_arena*         TempArena)
{
    ...
    {for(int Idx = 0; Idx < (TypeCount-1); ++Idx)
    {
			 ...
			 && VertexType[Idx].ArrayWidth == 3 )
        {
            ...
            // compressed normal
            NewType[Idx].Type = GrannyInt8Member; //GrannyReal16Member;
...
#undef MakePacked_10_11_11*/
	...
	nY /= length;
	nZ /= length;
	Comp[0] = (signed char)(nX * (1 << 6));
	Comp[1] = (signed char)(nY * (1 << 6));
	Comp[2] = (signed char)(nZ * (1 << 6));
//		{for(int j = 0; j < 3; ++j)
//        {
//			// wii fixed point 1:14
//            Comp[j] = (short)(Norm[j] * ( 1 << 14 ));
                 ...
                 void* NewVertices, int OffsetInComp, int StrideComp)
{
    ...
    {for(int Idx = 0; Idx < VertexCount; ++Idx)
    {
        short* Comp = (short*)Compressed;

        {for(int j = 0; j < NumComponents; ++j)
        {
			// wii fixed point 5:10
			float tcoord = Original[j];

			// range hack, assuming wrapping behavior if it's this outrageous
			if (tcoord > 31.0f)
			{
				tcoord = tcoord - ( tcoord - 31.0f );
			}
			else if (tcoord < -31.0f)
			{
				tcoord = tcoord - ( tcoord + 31.0f );
			}
            Comp[j] = (short)(Original[j] * ( 1 << 10 ));
        }}
...
granny_file_info*
CompressVertSampleWii(char const*          OriginalFilename,
...
static CommandRegistrar RegCompressVertSample(CompressVertSampleWii, "CompressVertSampleWii",
                                              "Compress normals and texture coordinates, wii version");
