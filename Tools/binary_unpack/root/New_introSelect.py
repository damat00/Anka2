...
class MyCharacters :
	class MyUnit :
		...
		if app.ENABLE_YOHARA_SYSTEM:
			def __init__(self, const_id, name, level, race, playtime, guildname, form, hair, acce, aura, stat_str, stat_dex, stat_hth, stat_int, change_name, sungma_level, sungma_str, sungma_hp, sungma_move, sungma_immune):
				self.UnitDataDic = {
					"ID" 			:	const_id,
					"NAME"			:	name,
					"LEVEL"			:	level,
	...
	def LoadCharacterData(self) :
		...
		for i in xrange(net.CHARACTER_SLOT_COUNT_MAX) :
			...
			if app.ENABLE_YOHARA_SYSTEM:
				...
				sungma_str 			=net.GetAccountCharacterSlotDataInteger(i, net.ACCOUNT_CHARACTER_SLOT_SUNGMA_STR)
			...
			if app.ENABLE_YOHARA_SYSTEM:
				self.myUnitDic[i] = self.MyUnit(i, name, level, race, playtime, guildname, form, hair, acce, aura, stat_str, stat_dex, stat_hth, stat_int, change_name, sungma_level, sungma_str, sungma_hp, sungma_move, sungma_immune)
			else:
...
class SelectCharacterWindow(ui.Window) :
	...
	def SelectButton(self, slot):
		...
		# event.SetFontColor(self.descIndex, 0.7843, 0.7843, 0.7843)  # event modülünde SetFontColor metodu yok
