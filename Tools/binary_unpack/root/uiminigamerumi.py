...
class CardsInfoWindow(ui.ScriptWindow):
	class DescriptionBox(ui.Window):
		def __init__(self):
			...
			self.SetWindowName("CardsInfoWindowDescriptionBox")
			self.descIndex = 0
	...
	def __init__(self):
		...
		self.SetWindowName("CardsInfoWindow")
		self.LoadWindow()
		self.descIndex=0
		self.scrollPos = 0
		self.safemode = 1
	...
	def LoadWindow(self):
		try:
			...
			import exception
			exception.Abort("CardsInfoWindow.LoadDialog.LoadScript")
		...
		try:
			...
			import exception
			exception.Abort("CardsInfoWindow.LoadDialog.BindObject")

		self.titleBar.SetCloseEvent(ui.__mem_func__(self.__OnCloseButtonClick))
		self.scrollBar.SetPos(0.0)
		self.scrollBar.SetScrollEvent(ui.__mem_func__(self.OnScroll))

		self.checkButton.SetEvent(ui.__mem_func__(self.__OnSafeMode), "mouse_click")
		self.checkButton2.SetEvent(ui.__mem_func__(self.__OnSafeMode), "mouse_click")
	...
	def __OnStartQuestion(self):
		...
		questionDialog.SetText(localeInfo.MINI_GAME_RUMI_START_QUESTION % (5000000, 1))
		questionDialog.SetAcceptEvent(ui.__mem_func__(self.__OnStart))
		questionDialog.SetCancelEvent(ui.__mem_func__(self.OnCloseQuestionDialog))
		questionDialog.SetWidth(500)
		questionDialog.Open()
