...
class GemTicketWindow(ui.ScriptWindow):
	def __init__(self):
	...
	def __LoadWindow(self):
		try:
			...
			pyScrLoader.LoadScriptFile(self, "UIScript/GemTicketWindow.py")
		except:
			import exception
	...
	def __OnClickAcceptButton(self):
		...
		if (len(self.inputValue.GetText()) == 0):
			chat.AppendChat(chat.CHAT_TYPE_INFO, localeInfo.USE_GEM_TICKET_EMPTY)
			return
		
		value = int(self.inputValue.GetText())
		if (value > player.GetGem()):
			chat.AppendChat(chat.CHAT_TYPE_INFO, localeInfo.USE_GEM_TICKET_NOT_ENOUGH)
			return
	...
	def __OnClickConfirmButton(self):
		...
		net.SendChatPacket("/use_gem_ticket {0} {1}".format(self.itemPos, int(self.inputValue.GetText())))
		self.Close()
