...
class CatchKingWaitingPage(ui.ScriptWindow):
	...
	def Show(self):
		...
		event.ClearEventSet(self.descIndex)
		self.descIndex = event.RegisterEventSet("locale/tr/catchking_event_desc.txt")
...
class CatchKingSimpleWaitingPage(ui.ScriptWindow):
	...
	def __ClickStartButton(self):
		...
		if len(startText) > 1:
			...
			self.startQuestionDialog.SetText2("")
	...
	def Show(self):
		...
		event.ClearEventSet(self.descIndex)
		self.descIndex = event.RegisterEventSet("locale/tr/catchking_event_simple_desc.txt")
		event.SetVisibleLineCount(self.descIndex, SHOW_LINE_COUNT_MAX)
		event.SetRestrictedCount(self.descIndex, 70)
		event.Skip(self.descIndex)
